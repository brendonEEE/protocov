...
* functions.
*/
uint8 ProtocovUSB_initVar = 0u;

#if (ProtocovUSB_EP_MANAGEMENT_DMA)
    #if (CY_PSOC4)
        static void ProtocovUSB_InitEpDma(void);

        /* DMA chanels assigend for endpoints. */
        const uint8 ProtocovUSB_DmaChan[ProtocovUSB_MAX_EP] =
        {
            0u,
            0u,
            0u,
    ...
    #else
        /* DMA chanels assigend for endpoints. */
        uint8 ProtocovUSB_DmaChan[ProtocovUSB_MAX_EP];
        
        /* DMA TDs require for PSoC 3/5LP operation. */
        uint8 ProtocovUSB_DmaTd[ProtocovUSB_MAX_EP];
    #endif /* (CY_PSOC4) */
    ...
    uint8  ProtocovUSB_DmaEpBurstCnt   [ProtocovUSB_MAX_EP];
    
    /* Number of bytes to transfer in last DMA burst. */
    uint8  ProtocovUSB_DmaEpLastBurstEl[ProtocovUSB_MAX_EP];

    /* Storage for arrays above. */
    uint8  ProtocovUSB_DmaEpBurstCntBackup  [ProtocovUSB_MAX_EP];
    uint32 ProtocovUSB_DmaEpBufferAddrBackup[ProtocovUSB_MAX_EP];
     
    /* DMA trigger mux output for usb.dma_req[0-7]. */
    const uint8 ProtocovUSB_DmaReqOut[ProtocovUSB_MAX_EP] =
    {
        0u,
        ProtocovUSB_ep1_dma__TR_OUTPUT,
        0u,
        0u,
        0u,
        0u,
    ...
    const uint8 ProtocovUSB_DmaBurstEndOut[ProtocovUSB_MAX_EP] =
    {
        0u,
        ProtocovUSB_BURSTEND_0_TR_OUTPUT,
        ProtocovUSB_BURSTEND_1_TR_OUTPUT,
        ProtocovUSB_BURSTEND_2_TR_OUTPUT,
        ProtocovUSB_BURSTEND_3_TR_OUTPUT,
        ProtocovUSB_BURSTEND_4_TR_OUTPUT,
        ProtocovUSB_BURSTEND_5_TR_OUTPUT,
        ProtocovUSB_BURSTEND_6_TR_OUTPUT,
        ProtocovUSB_BURSTEND_7_TR_OUTPUT
    };
    
#else
    #if (ProtocovUSB_EP_DMA_AUTO_OPT == 0u)
        static uint8 clearInDataRdyStatus = ProtocovUSB_ARB_EPX_CFG_DEFAULT;
        uint8 ProtocovUSB_DmaNextTd[ProtocovUSB_MAX_EP];
        const uint8 ProtocovUSB_epX_TD_TERMOUT_EN[ProtocovUSB_MAX_EP] =
        {
            0u,
            0u,
            0u,
        ...
        volatile uint16 ProtocovUSB_inLength[ProtocovUSB_MAX_EP];
        const uint8 *ProtocovUSB_inDataPointer[ProtocovUSB_MAX_EP];
        volatile uint8 ProtocovUSB_inBufFull[ProtocovUSB_MAX_EP];
...
/*******************************************************************************
* Function Name: ProtocovUSB_Start
****************************************************************************//**
*
...
*   following list.
*
*       *ProtocovUSB_3V_OPERATION* - Disable voltage regulator and pass-
*                                      through Vcc for pull-up
*
*       *ProtocovUSB_5V_OPERATION* - Enable voltage regulator and use
*                                      regulator for pull-up
*
*       *ProtocovUSB_DWR_POWER_OPERATION* - Enable or disable the voltage
...
{
    if (0u == ProtocovUSB_initVar)
    {
        ProtocovUSB_Init();
        ProtocovUSB_initVar = 1u;
    }

    ProtocovUSB_InitComponent(device, mode);
}


/*******************************************************************************
* Function Name: ProtocovUSB_Init
****************************************************************************//**
*
...
* ProtocovUSB_Init() because the ProtocovUSB_Start() routine calls
...
void ProtocovUSB_Init(void) 
{
    ...
    * ProtocovUSB_InitComponent() if it is required.
    */

    /* Enable USBIO control on drive mode of D+ and D- pins. */
    ProtocovUSB_USBIO_CR1_REG &= ~ (uint32) ProtocovUSB_USBIO_CR1_IOMODE;

    /* Set number of LF CLK to detect UBS bus reset. */
    ProtocovUSB_BUS_RST_CNT_REG = ProtocovUSB_DEFUALT_BUS_RST_CNT;
    ...
    */
    ProtocovUSB_POWER_CTRL_REG = ProtocovUSB_DEFAULT_POWER_CTRL_VBUS;

    /* Enable PHY detector and single-ended and differential receivers. */
    ProtocovUSB_POWER_CTRL_REG |= ProtocovUSB_DEFAULT_POWER_CTRL_PHY;
    ...
    ProtocovUSB_CR1_REG = ProtocovUSB_DEFUALT_CR1;
    ...
    ProtocovUSB_INTR_SIE_MASK_REG = ProtocovUSB_DEFAULT_INTR_SIE_MASK;
    ...
    ProtocovUSB_CR1_REG = ProtocovUSB_CR1_ENABLE_LOCK;
    ...
    */
    ...
    ProtocovUSB_USBIO_CR1_REG &= (uint8) ~ProtocovUSB_USBIO_CR1_IOMODE;
    ...
    CyDelayUs(ProtocovUSB_WAIT_REG_STABILITY_1US);
    ...
    ProtocovUSB_DM_INP_DIS_REG &= (uint8) ~ProtocovUSB_DM_MASK;
    ProtocovUSB_DP_INP_DIS_REG &= (uint8) ~ProtocovUSB_DP_MASK;
    ...
    CY_SET_REG8(ProtocovUSB_ARB_RW1_WA_PTR,     0u);
    CY_SET_REG8(ProtocovUSB_ARB_RW1_WA_MSB_PTR, 0u);

#if (ProtocovUSB_EP_MANAGEMENT_DMA)
    /* Initialize transfer descriptor. This will be used to detect DMA state - initialized or not. */
    for (i = 0u; i < ProtocovUSB_MAX_EP; ++i)
    {
        ProtocovUSB_DmaTd[i] = DMA_INVALID_TD;

    #if (ProtocovUSB_EP_MANAGEMENT_DMA_AUTO && (ProtocovUSB_EP_DMA_AUTO_OPT == 0u))
        ProtocovUSB_DmaNextTd[i] = DMA_INVALID_TD;
    ...
    CyIntSetPriority     (ProtocovUSB_INTR_HI_VECT_NUM,  ProtocovUSB_INTR_HI_PRIORITY);
    (void) CyIntSetVector(ProtocovUSB_INTR_HI_VECT_NUM, &ProtocovUSB_INTR_HI_ISR);

    /* Configure lo_int: set handler and priority. */
    CyIntSetPriority     (ProtocovUSB_INTR_LO_VECT_NUM,  ProtocovUSB_INTR_LO_PRIORITY);
    (void) CyIntSetVector(ProtocovUSB_INTR_LO_VECT_NUM, &ProtocovUSB_INTR_LO_ISR);

    /* Configure med_int: set handler and priority (routed through DSI). */
    CyIntSetPriority     (ProtocovUSB_INTR_MED_VECT_NUM,  ProtocovUSB_INTR_MED_PRIORITY);
    (void) CyIntSetVector(ProtocovUSB_INTR_MED_VECT_NUM, &ProtocovUSB_INTR_MED_ISR);

#else
    /* Set bus reset interrupt. */
    CyIntSetPriority(ProtocovUSB_BUS_RESET_VECT_NUM, ProtocovUSB_BUS_RESET_PRIOR);
    (void) CyIntSetVector(ProtocovUSB_BUS_RESET_VECT_NUM,   &ProtocovUSB_BUS_RESET_ISR);

    /* Set Control Endpoint Interrupt. */
    CyIntSetPriority(ProtocovUSB_EP_0_VECT_NUM, ProtocovUSB_EP_0_PRIOR);
    (void) CyIntSetVector(ProtocovUSB_EP_0_VECT_NUM,   &ProtocovUSB_EP_0_ISR);

    /* Set SOF interrupt. */
    #if (ProtocovUSB_SOF_ISR_ACTIVE)
        CyIntSetPriority     (ProtocovUSB_SOF_VECT_NUM,  ProtocovUSB_SOF_PRIOR);
        (void) CyIntSetVector(ProtocovUSB_SOF_VECT_NUM, &ProtocovUSB_SOF_ISR);
    #endif /* (ProtocovUSB_SOF_ISR_ACTIVE) */

    /* Set Data Endpoint 1 Interrupt. */
    #if (ProtocovUSB_EP1_ISR_ACTIVE)
        CyIntSetPriority     (ProtocovUSB_EP_1_VECT_NUM,  ProtocovUSB_EP_1_PRIOR);
        (void) CyIntSetVector(ProtocovUSB_EP_1_VECT_NUM,  &ProtocovUSB_EP_1_ISR);
    #endif /* (ProtocovUSB_EP1_ISR_ACTIVE) */

    /* Set Data Endpoint 2 Interrupt. */
    #if (ProtocovUSB_EP2_ISR_ACTIVE)
        CyIntSetPriority     (ProtocovUSB_EP_2_VECT_NUM,  ProtocovUSB_EP_2_PRIOR);
        (void) CyIntSetVector(ProtocovUSB_EP_2_VECT_NUM, &ProtocovUSB_EP_2_ISR);
    #endif /* (ProtocovUSB_EP2_ISR_ACTIVE) */

    /* Set Data Endpoint 3 Interrupt. */
    #if (ProtocovUSB_EP3_ISR_ACTIVE)
        CyIntSetPriority     (ProtocovUSB_EP_3_VECT_NUM,  ProtocovUSB_EP_3_PRIOR);
        (void) CyIntSetVector(ProtocovUSB_EP_3_VECT_NUM, &ProtocovUSB_EP_3_ISR);
    #endif /* (ProtocovUSB_EP3_ISR_ACTIVE) */

    /* Set Data Endpoint 4 Interrupt. */
    #if (ProtocovUSB_EP4_ISR_ACTIVE)
        CyIntSetPriority     (ProtocovUSB_EP_4_VECT_NUM,  ProtocovUSB_EP_4_PRIOR);
        (void) CyIntSetVector(ProtocovUSB_EP_4_VECT_NUM, &ProtocovUSB_EP_4_ISR);
    #endif /* (ProtocovUSB_EP4_ISR_ACTIVE) */

    /* Set Data Endpoint 5 Interrupt. */
    #if (ProtocovUSB_EP5_ISR_ACTIVE)
        CyIntSetPriority     (ProtocovUSB_EP_5_VECT_NUM,  ProtocovUSB_EP_5_PRIOR);
        (void) CyIntSetVector(ProtocovUSB_EP_5_VECT_NUM, &ProtocovUSB_EP_5_ISR);
    #endif /* (ProtocovUSB_EP5_ISR_ACTIVE) */

    /* Set Data Endpoint 6 Interrupt. */
    #if (ProtocovUSB_EP6_ISR_ACTIVE)
        CyIntSetPriority     (ProtocovUSB_EP_6_VECT_NUM,  ProtocovUSB_EP_6_PRIOR);
        (void) CyIntSetVector(ProtocovUSB_EP_6_VECT_NUM, &ProtocovUSB_EP_6_ISR);
    #endif /* (ProtocovUSB_EP6_ISR_ACTIVE) */

     /* Set Data Endpoint 7 Interrupt. */
    #if (ProtocovUSB_EP7_ISR_ACTIVE)
        CyIntSetPriority     (ProtocovUSB_EP_7_VECT_NUM,  ProtocovUSB_EP_7_PRIOR);
        (void) CyIntSetVector(ProtocovUSB_EP_7_VECT_NUM, &ProtocovUSB_EP_7_ISR);
    #endif /* (ProtocovUSB_EP7_ISR_ACTIVE) */

    /* Set Data Endpoint 8 Interrupt. */
    #if (ProtocovUSB_EP8_ISR_ACTIVE)
        CyIntSetPriority     (ProtocovUSB_EP_8_VECT_NUM,  ProtocovUSB_EP_8_PRIOR);
        (void) CyIntSetVector(ProtocovUSB_EP_8_VECT_NUM, &ProtocovUSB_EP_8_ISR);
    #endif /* (ProtocovUSB_EP8_ISR_ACTIVE) */

    /* Set ARB Interrupt. */
    #if (ProtocovUSB_EP_MANAGEMENT_DMA && ProtocovUSB_ARB_ISR_ACTIVE)
        CyIntSetPriority     (ProtocovUSB_ARB_VECT_NUM,  ProtocovUSB_ARB_PRIOR);
        (void) CyIntSetVector(ProtocovUSB_ARB_VECT_NUM, &ProtocovUSB_ARB_ISR);
    #endif /* (ProtocovUSB_EP_MANAGEMENT_DMA && ProtocovUSB_ARB_ISR_ACTIVE) */
#endif /* (CY_PSOC4) */

    /* Common: Configure GPIO interrupt for wakeup. */
#if (ProtocovUSB_DP_ISR_ACTIVE)
    CyIntSetPriority     (ProtocovUSB_DP_INTC_VECT_NUM,  ProtocovUSB_DP_INTC_PRIORITY);
    (void) CyIntSetVector(ProtocovUSB_DP_INTC_VECT_NUM, &ProtocovUSB_DP_ISR);
#endif /* (ProtocovUSB_DP_ISR_ACTIVE) */

#if (ProtocovUSB_EP_MANAGEMENT_DMA && CY_PSOC4)
    /* Initialize DMA channels. */
    ProtocovUSB_InitEpDma();
#endif /* (ProtocovUSB_EP_MANAGEMENT_DMA && CY_PSOC4) */
}


/*******************************************************************************
* Function Name: ProtocovUSB_InitComponent
****************************************************************************//**
*
...
*
* \globalvars
*   \ref ProtocovUSB_device
*   \ref ProtocovUSB_transferState
*   \ref ProtocovUSB_configuration
*   \ref ProtocovUSB_deviceStatus
*
*   \ref ProtocovUSB_deviceAddress - Contains the current device address. This
...
*      SET_ADDRESS request.
*
*   \ref ProtocovUSB_lastPacketSize - Initialized to 0;
*
* \reentrant
*  No.
*
...
void ProtocovUSB_InitComponent(uint8 device, uint8 mode) 
{
    ...
    */
    ...
    {
        ProtocovUSB_hidProtocol[i] = ProtocovUSB_PROTOCOL_REPORT;
    }
#endif /* ProtocovUSB_ENABLE_HID_CLASS */

    /* Store device number to access descriptor. */
    ProtocovUSB_device = device;

    /* Reset component internal variables. */
    ProtocovUSB_transferState   = ProtocovUSB_TRANS_STATE_IDLE;
    ProtocovUSB_configurationChanged = 0u;
    ProtocovUSB_configuration   = 0u;
    ProtocovUSB_interfaceNumber = 0u;
    ProtocovUSB_deviceAddress   = 0u;
    ProtocovUSB_deviceStatus    = 0u;
    ProtocovUSB_lastPacketSize  = 0u;

    /* Enable component interrupts. */
#if (CY_PSOC4)
    CyIntEnable(ProtocovUSB_INTR_HI_VECT_NUM);
    CyIntEnable(ProtocovUSB_INTR_MED_VECT_NUM);
    CyIntEnable(ProtocovUSB_INTR_LO_VECT_NUM);
#else
    CyIntEnable(ProtocovUSB_BUS_RESET_VECT_NUM);
    CyIntEnable(ProtocovUSB_EP_0_VECT_NUM);

    #if (ProtocovUSB_SOF_ISR_ACTIVE)
        CyIntEnable(ProtocovUSB_SOF_VECT_NUM);
    #endif /* (ProtocovUSB_SOF_ISR_ACTIVE) */

    #if (ProtocovUSB_EP1_ISR_ACTIVE)
        CyIntEnable(ProtocovUSB_EP_1_VECT_NUM);
    #endif /* (ProtocovUSB_EP1_ISR_ACTIVE) */

    #if (ProtocovUSB_EP2_ISR_ACTIVE)
        CyIntEnable(ProtocovUSB_EP_2_VECT_NUM);
    #endif /* (ProtocovUSB_EP5_ISR_ACTIVE) */

    #if (ProtocovUSB_EP3_ISR_ACTIVE)
        CyIntEnable(ProtocovUSB_EP_3_VECT_NUM);
    #endif /* (ProtocovUSB_EP5_ISR_ACTIVE) */

    #if (ProtocovUSB_EP4_ISR_ACTIVE)
        CyIntEnable(ProtocovUSB_EP_4_VECT_NUM);
    #endif /* (ProtocovUSB_EP5_ISR_ACTIVE) */

    #if (ProtocovUSB_EP5_ISR_ACTIVE)
        CyIntEnable(ProtocovUSB_EP_5_VECT_NUM);
    #endif /* (ProtocovUSB_EP5_ISR_ACTIVE) */

    #if (ProtocovUSB_EP6_ISR_ACTIVE)
        CyIntEnable(ProtocovUSB_EP_6_VECT_NUM);
    #endif /* ProtocovUSB_EP6_ISR_REMOVE */

    #if (ProtocovUSB_EP7_ISR_ACTIVE)
        CyIntEnable(ProtocovUSB_EP_7_VECT_NUM);
    #endif /* (ProtocovUSB_EP7_ISR_ACTIVE) */

    #if (ProtocovUSB_EP8_ISR_ACTIVE)
        CyIntEnable(ProtocovUSB_EP_8_VECT_NUM);
    #endif /* (ProtocovUSB_EP8_ISR_ACTIVE) */
#endif /* (CY_PSOC4) */
    ...
    ProtocovUSB_ARB_INT_EN_REG = ProtocovUSB_DEFAULT_ARB_INT_EN;
        ...
        CyIntEnable(ProtocovUSB_ARB_VECT_NUM);
    ...
    ProtocovUSB_ARB_CFG_REG = ProtocovUSB_DEFAULT_ARB_CFG;
        ...
        CyDmaEnable();
            ...
            /* Change DMA priority to be highest. */
             CyIntSetPriority(CYDMA_INTR_NUMBER, ProtocovUSB_DMA_AUTO_INTR_PRIO);
        #endif /* (ProtocovUSB_EP_MANAGEMENT_DMA_AUTO) */
    #endif /* (CY_PSOC4) */
            ...
            CyIntEnable(CYDMA_INTR_NUMBER);
                ...
                ProtocovUSB_EP_DMA_Done_isr_StartEx(&ProtocovUSB_EP_DMA_DONE_ISR);
                ProtocovUSB_EP17_DMA_Done_SR_InterruptEnable();
                ProtocovUSB_EP8_DMA_Done_SR_InterruptEnable();
            #endif /* (ProtocovUSB_EP_DMA_AUTO_OPT == 0u) */
        #endif /* (CY_PSOC4) */
    #endif /* (ProtocovUSB_EP_MANAGEMENT_DMA_AUTO) */
#endif /* (ProtocovUSB_EP_MANAGEMENT_DMA) */
...
/*******************************************************************************
* Function Name: ProtocovUSB_ReInitComponent
****************************************************************************//**
*
...
*
* \globalvars
*   ProtocovUSB_device - Contains the device number of the desired Device
...
*   ProtocovUSB_lastPacketSize - Initialized to 0;
*
* \reentrant
*  No.
*
...
/*******************************************************************************
* Function Name: ProtocovUSB_Stop
****************************************************************************//**
*
...
*   \ref ProtocovUSB_deviceStatus
*
*   \ref ProtocovUSB_configurationChanged
*
*   ProtocovUSB_intiVar -  This variable is set to zero
*
*******************************************************************************/
void ProtocovUSB_Stop(void) 
{
    ...
    /* Disable D+ pull-up. */
    ProtocovUSB_USBIO_CR1_REG &= (uint8) ~ ProtocovUSB_USBIO_CR1_USBPUEN;

#if (CY_PSOC4)
    /* Disable USBFS block. */
    ProtocovUSB_POWER_CTRL_REG &= (uint32) ~ProtocovUSB_POWER_CTRL_ENABLE;
#else
...
#if (CY_PSOC4)
    CyIntDisable(ProtocovUSB_INTR_HI_VECT_NUM);
    CyIntDisable(ProtocovUSB_INTR_LO_VECT_NUM);
    CyIntDisable(ProtocovUSB_INTR_MED_VECT_NUM);
#else

    CyIntDisable(ProtocovUSB_BUS_RESET_VECT_NUM);
    CyIntDisable(ProtocovUSB_EP_0_VECT_NUM);

    #if (ProtocovUSB_SOF_ISR_ACTIVE)
        CyIntDisable(ProtocovUSB_SOF_VECT_NUM);
    #endif /* (ProtocovUSB_SOF_ISR_ACTIVE) */

    #if (ProtocovUSB_EP1_ISR_ACTIVE)
        CyIntDisable(ProtocovUSB_EP_1_VECT_NUM);
    #endif /* (ProtocovUSB_EP1_ISR_ACTIVE) */

    #if (ProtocovUSB_EP2_ISR_ACTIVE)
        CyIntDisable(ProtocovUSB_EP_2_VECT_NUM);
    #endif /* (ProtocovUSB_EP2_ISR_ACTIVE) */

    #if (ProtocovUSB_EP3_ISR_ACTIVE)
        CyIntDisable(ProtocovUSB_EP_3_VECT_NUM);
    #endif /* (ProtocovUSB_EP3_ISR_ACTIVE) */

    #if (ProtocovUSB_EP4_ISR_ACTIVE)
        CyIntDisable(ProtocovUSB_EP_4_VECT_NUM);
    #endif /* (ProtocovUSB_EP4_ISR_ACTIVE) */

    #if (ProtocovUSB_EP5_ISR_ACTIVE)
        CyIntDisable(ProtocovUSB_EP_5_VECT_NUM);
    #endif /* (ProtocovUSB_EP5_ISR_ACTIVE) */

    #if (ProtocovUSB_EP6_ISR_ACTIVE)
        CyIntDisable(ProtocovUSB_EP_6_VECT_NUM);
    #endif /* ProtocovUSB_EP6_ISR_REMOVE */

    #if (ProtocovUSB_EP7_ISR_ACTIVE)
        CyIntDisable(ProtocovUSB_EP_7_VECT_NUM);
    #endif /* (ProtocovUSB_EP7_ISR_ACTIVE) */

    #if (ProtocovUSB_EP8_ISR_ACTIVE)
        CyIntDisable(ProtocovUSB_EP_8_VECT_NUM);
    #endif /* (ProtocovUSB_EP8_ISR_ACTIVE) */
    ...
    ProtocovUSB_initVar = 0u;
}


/*******************************************************************************
* Function Name: ProtocovUSB_CheckActivity
****************************************************************************//**
*
...
{
    uint8 cr1Reg = ProtocovUSB_CR1_REG;

    /* Clear bus activity. */
    ProtocovUSB_CR1_REG = (cr1Reg & (uint8) ~ProtocovUSB_CR1_BUS_ACTIVITY);

    /* Get bus activity. */
    return ((0u != (cr1Reg & ProtocovUSB_CR1_BUS_ACTIVITY)) ? (1u) : (0u));
}


/*******************************************************************************
* Function Name: ProtocovUSB_GetConfiguration
****************************************************************************//**
*
...
{
    return (ProtocovUSB_configuration);
}


/*******************************************************************************
* Function Name: ProtocovUSB_IsConfigurationChanged
****************************************************************************//**
*
...
* \globalvars
*
*  \ref ProtocovUSB_configurationChanged - This variable is set to 1 after
...
uint8 ProtocovUSB_IsConfigurationChanged(void) 
{
    ...
    if (ProtocovUSB_configurationChanged != 0u)
    {
        res = ProtocovUSB_configurationChanged;
        ProtocovUSB_configurationChanged = 0u;
    }
...
/*******************************************************************************
* Function Name: ProtocovUSB_GetInterfaceSetting
****************************************************************************//**
*
...
{
    return (ProtocovUSB_interfaceSetting[interfaceNumber]);
}


/*******************************************************************************
* Function Name: ProtocovUSB_GetEPState
****************************************************************************//**
*
...
{
    return (ProtocovUSB_EP[epNumber].apiEpState);
}


/*******************************************************************************
* Function Name: ProtocovUSB_GetEPCount
****************************************************************************//**
*
...
uint16 ProtocovUSB_GetEPCount(uint8 epNumber) 
{
    ...
    if ((epNumber > ProtocovUSB_EP0) && (epNumber < ProtocovUSB_MAX_EP))
    {
        ...
        cntr  = ((uint16) ProtocovUSB_SIE_EP_BASE.sieEp[epNumber].epCnt0) & ProtocovUSB_EPX_CNT0_MASK;
        cntr  = ((uint16) (cntr << 8u)) | ((uint16) ProtocovUSB_SIE_EP_BASE.sieEp[epNumber].epCnt1);
        cntr -= ProtocovUSB_EPX_CNTX_CRC_COUNT;
    }
...
#if (CY_PSOC4)
    /*******************************************************************************
    * Function Name: ProtocovUSB_InitEpDma
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    static void ProtocovUSB_InitEpDma(void)
    {
        ...
        CYDMA_CH_CTL_BASE.ctl[ProtocovUSB_ep1_dma_CHANNEL] = ProtocovUSB_ep1_dma_CHANNEL_CFG;
    #endif /* (ProtocovUSB_DMA1_ACTIVE) */

    #if (ProtocovUSB_DMA2_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[ProtocovUSB_ep2_dma_CHANNEL] = ProtocovUSB_ep2_dma_CHANNEL_CFG;
    #endif /* (ProtocovUSB_DMA2_ACTIVE) */

    #if (ProtocovUSB_DMA3_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[ProtocovUSB_ep3_dma_CHANNEL] = ProtocovUSB_ep3_dma_CHANNEL_CFG;
    #endif /* (ProtocovUSB_DMA3_ACTIVE) */

    #if (ProtocovUSB_DMA4_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[ProtocovUSB_ep4_dma_CHANNEL] = ProtocovUSB_ep4_dma_CHANNEL_CFG;
    #endif /* (ProtocovUSB_DMA4_ACTIVE) */

    #if (ProtocovUSB_DMA5_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[ProtocovUSB_ep5_dma_CHANNEL] = ProtocovUSB_ep5_dma_CHANNEL_CFG;
    #endif /* (ProtocovUSB_DMA5_ACTIVE) */

    #if (ProtocovUSB_DMA6_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[ProtocovUSB_ep6_dma_CHANNEL] = ProtocovUSB_ep6_dma_CHANNEL_CFG;
    #endif /* (ProtocovUSB_DMA6_ACTIVE) */

    #if (ProtocovUSB_DMA7_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[ProtocovUSB_ep7_dma_CHANNEL] = ProtocovUSB_ep7_dma_CHANNEL_CFG;
    #endif /* (ProtocovUSB_DMA7_ACTIVE) */

    #if (ProtocovUSB_DMA8_ACTIVE)
        CYDMA_CH_CTL_BASE.ctl[ProtocovUSB_ep8_dma_CHANNEL] = ProtocovUSB_ep8_dma_CHANNEL_CFG;
    #endif /* (ProtocovUSB_DMA8_ACTIVE) */

    #if (ProtocovUSB_EP_MANAGEMENT_DMA_AUTO)
        /* Initialize DMA channel callbacks. */
        #if (ProtocovUSB_DMA1_ACTIVE)
            (void) ProtocovUSB_ep1_dma_SetInterruptCallback(&ProtocovUSB_EP1_DMA_DONE_ISR);
        #endif /* (ProtocovUSB_DMA1_ACTIVE) */

        #if (ProtocovUSB_DMA2_ACTIVE)
            (void) ProtocovUSB_ep2_dma_SetInterruptCallback(&ProtocovUSB_EP2_DMA_DONE_ISR);
        #endif /* (ProtocovUSB_DMA2_ACTIVE) */

        #if (ProtocovUSB_DMA3_ACTIVE)
            (void) ProtocovUSB_ep3_dma_SetInterruptCallback(&ProtocovUSB_EP3_DMA_DONE_ISR);
        #endif /* (ProtocovUSB_DMA3_ACTIVE) */

        #if (ProtocovUSB_DMA4_ACTIVE)
            (void) ProtocovUSB_ep4_dma_SetInterruptCallback(&ProtocovUSB_EP4_DMA_DONE_ISR);
        #endif /* (ProtocovUSB_DMA4_ACTIVE) */

        #if (ProtocovUSB_DMA5_ACTIVE)
            (void) ProtocovUSB_ep5_dma_SetInterruptCallback(&ProtocovUSB_EP5_DMA_DONE_ISR);
        #endif /* (ProtocovUSB_DMA5_ACTIVE) */

        #if (ProtocovUSB_DMA6_ACTIVE)
            (void) ProtocovUSB_ep6_dma_SetInterruptCallback(&ProtocovUSB_EP6_DMA_DONE_ISR);
        #endif /* (ProtocovUSB_DMA6_ACTIVE) */

        #if (ProtocovUSB_DMA7_ACTIVE)
            (void) ProtocovUSB_ep7_dma_SetInterruptCallback(&ProtocovUSB_EP7_DMA_DONE_ISR);
        #endif /* (ProtocovUSB_DMA7_ACTIVE) */

        #if (ProtocovUSB_DMA8_ACTIVE)
            (void) ProtocovUSB_ep8_dma_SetInterruptCallback(&ProtocovUSB_EP8_DMA_DONE_ISR);
        #endif /* (ProtocovUSB_DMA8_ACTIVE) */
    #endif /* (ProtocovUSB_EP_MANAGEMENT_DMA_AUTO) */
    }
#else
    ...
    /***************************************************************************
    * Function Name: ProtocovUSB_InitEP_DMA
    ************************************************************************//**
    *
    *  This function allocates and initializes a DMA channel to be used by the
    *  ProtocovUSB_LoadInEP() or ProtocovUSB_ReadOutEP() APIs for data
    ...
    ***************************************************************************/
                                                                    ...
                                                                    
    {
        ...
        {
    #if (ProtocovUSB_DMA1_ACTIVE)
        case ProtocovUSB_EP1:
            ProtocovUSB_DmaChan[epNumber] = ProtocovUSB_ep1_DmaInitialize(ProtocovUSB_DMA_BYTES_PER_BURST,
                                                                                    ProtocovUSB_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (ProtocovUSB_DMA1_ACTIVE) */

    #if (ProtocovUSB_DMA2_ACTIVE)
        case ProtocovUSB_EP2:
            ProtocovUSB_DmaChan[epNumber] = ProtocovUSB_ep2_DmaInitialize(ProtocovUSB_DMA_BYTES_PER_BURST,
                                                                                    ProtocovUSB_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (ProtocovUSB_DMA2_ACTIVE) */

    #if (ProtocovUSB_DMA3_ACTIVE)
            case ProtocovUSB_EP3:
                ProtocovUSB_DmaChan[epNumber] = ProtocovUSB_ep3_DmaInitialize(ProtocovUSB_DMA_BYTES_PER_BURST,
                                                                                        ProtocovUSB_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (ProtocovUSB_DMA3_ACTIVE) */

    #if (ProtocovUSB_DMA4_ACTIVE)
            case ProtocovUSB_EP4:
                ProtocovUSB_DmaChan[epNumber] = ProtocovUSB_ep4_DmaInitialize(ProtocovUSB_DMA_BYTES_PER_BURST,
                                                                                        ProtocovUSB_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (ProtocovUSB_DMA4_ACTIVE) */

    #if (ProtocovUSB_DMA5_ACTIVE)
            case ProtocovUSB_EP5:
                ProtocovUSB_DmaChan[epNumber] = ProtocovUSB_ep5_DmaInitialize(ProtocovUSB_DMA_BYTES_PER_BURST,
                                                                                        ProtocovUSB_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (ProtocovUSB_DMA5_ACTIVE) */

    #if (ProtocovUSB_DMA6_ACTIVE)
        case ProtocovUSB_EP6:
            ProtocovUSB_DmaChan[epNumber] = ProtocovUSB_ep6_DmaInitialize(ProtocovUSB_DMA_BYTES_PER_BURST,
                                                                                    ProtocovUSB_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (ProtocovUSB_DMA6_ACTIVE) */

    #if (ProtocovUSB_DMA7_ACTIVE)
        case ProtocovUSB_EP7:
                ProtocovUSB_DmaChan[epNumber] = ProtocovUSB_ep7_DmaInitialize(ProtocovUSB_DMA_BYTES_PER_BURST,
                                                                                        ProtocovUSB_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (ProtocovUSB_DMA7_ACTIVE) */

    #if (ProtocovUSB_DMA8_ACTIVE)
        case ProtocovUSB_EP8:
                ProtocovUSB_DmaChan[epNumber] = ProtocovUSB_ep8_DmaInitialize(ProtocovUSB_DMA_BYTES_PER_BURST,
                                                                                        ProtocovUSB_DMA_REQUEST_PER_BURST, src, dst);
            break;
    #endif /* (ProtocovUSB_DMA8_ACTIVE) */
        ...
        if ((epNumber > ProtocovUSB_EP0) && (epNumber < ProtocovUSB_MAX_EP))
        {
            ProtocovUSB_DmaTd[epNumber] = CyDmaTdAllocate();

        #if (ProtocovUSB_EP_MANAGEMENT_DMA_AUTO && (ProtocovUSB_EP_DMA_AUTO_OPT == 0u))
            ProtocovUSB_DmaNextTd[epNumber] = CyDmaTdAllocate();
        #endif /* (ProtocovUSB_EP_MANAGEMENT_DMA_AUTO && (ProtocovUSB_EP_DMA_AUTO_OPT == 0u)) */
        }
    }
#endif /* (CY_PSOC4) */
    ...
    /***************************************************************************
    * Function Name: ProtocovUSB_Stop_DMA
    ************************************************************************//**
    *  
    ...
    *  to trigger DMA re-configuration when ProtocovUSB_LoadInEP() or 
    *  ProtocovUSB_ReadOutEP() functions are called the first time. 
    *  
    ...
    ***************************************************************************/
    void ProtocovUSB_Stop_DMA(uint8 epNumber) 
    {
        ...
        i = (epNumber < ProtocovUSB_MAX_EP) ? epNumber : ProtocovUSB_EP1;

        do
        {
        #if (CY_PSOC4)
            if (0u != ProtocovUSB_DmaChan[i])
            {
                ProtocovUSB_CyDmaChDisable(ProtocovUSB_DmaChan[i]);
            }
        #else
            if(ProtocovUSB_DmaTd[i] != DMA_INVALID_TD)
            {
                (void) CyDmaChDisable(ProtocovUSB_DmaChan[i]);
                CyDmaTdFree(ProtocovUSB_DmaTd[i]);
                ProtocovUSB_DmaTd[i] = DMA_INVALID_TD;
            }

            #if (ProtocovUSB_EP_MANAGEMENT_DMA_AUTO && (ProtocovUSB_EP_DMA_AUTO_OPT == 0u))
                if(ProtocovUSB_DmaNextTd[i] != DMA_INVALID_TD)
                {
                    CyDmaTdFree(ProtocovUSB_DmaNextTd[i]);
                    ProtocovUSB_DmaNextTd[i] = DMA_INVALID_TD;
                }
            #endif /* (ProtocovUSB_EP_MANAGEMENT_DMA_AUTO && (ProtocovUSB_EP_DMA_AUTO_OPT == 0u)) */
        ...
        while ((i < ProtocovUSB_MAX_EP) && (epNumber == ProtocovUSB_MAX_EP));
    }
#endif /* (ProtocovUSB_EP_MANAGEMENT_DMA) */
...
#if (ProtocovUSB_EP_MANAGEMENT_DMA_AUTO && (ProtocovUSB_EP_DMA_AUTO_OPT == 0u))
    /***************************************************************************
    * Function Name: ProtocovUSB_LoadNextInEP
    ************************************************************************//**
    *
    * Summary:
    ...
    ***************************************************************************/
    void ProtocovUSB_LoadNextInEP(uint8 epNumber, uint8 mode) 
    {
        ...
        if (mode == 0u)
        {
            ...
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[ProtocovUSB_DmaTd[epNumber]].TD0[0u];
            /* Set transfer length. */
            CY_SET_REG16(convert, ProtocovUSB_inLength[epNumber] - ProtocovUSB_DMA_BYTES_PER_BURST);
            /* CyDmaTdSetAddress API is optimized to change source address only. */
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[ProtocovUSB_DmaTd[epNumber]].TD1[0u];
            CY_SET_REG16(convert, LO16((uint32)ProtocovUSB_inDataPointer[epNumber] +
                                            ProtocovUSB_DMA_BYTES_PER_BURST));
            ProtocovUSB_inBufFull[epNumber] = 1u;
        }
        else
        {
            ...
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[ProtocovUSB_DmaTd[epNumber]].TD0[0u];
            /* Set transfer length. */
            CY_SET_REG16(convert, ProtocovUSB_DMA_BYTES_REPEAT);
            /* CyDmaTdSetAddress API is optimized to change source address only. */
            convert = (reg16 *) &CY_DMA_TDMEM_STRUCT_PTR[ProtocovUSB_DmaTd[epNumber]].TD1[0u];
            CY_SET_REG16(convert,  LO16((uint32)ProtocovUSB_inDataPointer[epNumber] +
                                   (ProtocovUSB_DMA_BYTES_PER_BURST - ProtocovUSB_DMA_BYTES_REPEAT)));
        }

        /* CyDmaChSetInitialTd API is optimized to initialize TD. */
        CY_DMA_CH_STRUCT_PTR[ProtocovUSB_DmaChan[epNumber]].basic_status[1u] = ProtocovUSB_DmaTd[epNumber];
    }
#endif /* (ProtocovUSB_EP_MANAGEMENT_DMA_AUTO && (ProtocovUSB_EP_DMA_AUTO_OPT == 0u)) */
#endif /* (CY_PSOC3 || CY_PSOC5) */
...
/*******************************************************************************
* Function Name: ProtocovUSB_LoadInEP
****************************************************************************//**
*
                                                                        ...
                                                                        
{
    if ((epNumber > ProtocovUSB_EP0) && (epNumber < ProtocovUSB_MAX_EP))
    {
        ...
        if (length > (ProtocovUSB_EPX_DATA_BUF_MAX - ProtocovUSB_EP[epNumber].buffOffset))
        {
            length = ProtocovUSB_EPX_DATA_BUF_MAX - ProtocovUSB_EP[epNumber].buffOffset;
        }
    #endif /* (!ProtocovUSB_EP_MANAGEMENT_DMA_AUTO) */

        /* Set count and data toggle. */
        ProtocovUSB_SIE_EP_BASE.sieEp[epNumber].epCnt0 = (uint8) HI8(length) | ProtocovUSB_EP[epNumber].epToggle;
        ProtocovUSB_SIE_EP_BASE.sieEp[epNumber].epCnt1 = (uint8) LO8(length);

    #if (ProtocovUSB_EP_MANAGEMENT_MANUAL)
        if (NULL != pData)
        {
            ...
            for (i = 0u; i < length; ++i)
            {
                ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].rwDr = pData[i];
            }
        }

        /* IN endpoint buffer is full - read to be read. */
        ProtocovUSB_EP[epNumber].apiEpState = ProtocovUSB_NO_EVENT_PENDING;

        /* Arm IN endpoint. */
        ProtocovUSB_SIE_EP_BASE.sieEp[epNumber].epCr0 = ProtocovUSB_EP[epNumber].epMode;

    #else

        #if (CY_PSOC3 || CY_PSOC5LP)
            /* Initialize DMA if it was not initialized. */
            if (DMA_INVALID_TD == ProtocovUSB_DmaTd[epNumber])
            {
                ProtocovUSB_InitEP_DMA(epNumber, pData);
            }
            ...
            {
            #if (CY_PSOC4)
                uint32 channelNum = (uint32) ProtocovUSB_DmaChan[epNumber];

                /* Disable DMA channel: start configuration. */
                ProtocovUSB_CyDmaChDisable(channelNum);

                /* Configure source and destination. */
                ProtocovUSB_CyDmaSetSrcAddress(channelNum, ProtocovUSB_DMA_DESCR0, (void*) pData);
                ProtocovUSB_CyDmaSetDstAddress(channelNum, ProtocovUSB_DMA_DESCR0, (void*) &ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].rwDr);

                /* Configure DMA descriptor. */
                --length;
                ProtocovUSB_CyDmaSetConfiguration(channelNum, ProtocovUSB_DMA_DESCR0, ProtocovUSB_DMA_COMMON_CFG | length |
                ...
                (void) CyDmaChDisable(ProtocovUSB_DmaChan[epNumber]);
                (void) CyDmaTdSetConfiguration(ProtocovUSB_DmaTd[epNumber], length, CY_DMA_DISABLE_TD, TD_TERMIN_EN | TD_INC_SRC_ADR);
                (void) CyDmaTdSetAddress(ProtocovUSB_DmaTd[epNumber], LO16((uint32) pData), LO16((uint32) &ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].rwDr));

                /* Enable DMA channel. */
                (void) CyDmaChSetInitialTd(ProtocovUSB_DmaChan[epNumber], ProtocovUSB_DmaTd[epNumber]);
                (void) CyDmaChEnable(ProtocovUSB_DmaChan[epNumber], 1u);
            #endif /* (CY_PSOC4) */

                /* Generate DMA request. */
                ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].epCfg |=  (uint8)  ProtocovUSB_ARB_EPX_CFG_DMA_REQ;
                ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].epCfg &=  (uint8) ~ProtocovUSB_ARB_EPX_CFG_DMA_REQ;
            ...
            if (pData != NULL)
            {
                ...
                ProtocovUSB_DmaEpBufferAddrBackup[epNumber] = (uint32) pData;
                ...
                ProtocovUSB_CyDmaSetDstAddress(channelNum, ProtocovUSB_DMA_DESCR0, (void*) &ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].rwDr);
                ProtocovUSB_CyDmaSetDstAddress(channelNum, ProtocovUSB_DMA_DESCR1, (void*) &ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].rwDr);

                /* Configure DMA descriptor. */
                ProtocovUSB_CyDmaSetConfiguration(channelNum, ProtocovUSB_DMA_DESCR0, ProtocovUSB_DMA_COMMON_CFG  |
                                                        CYDMA_BYTE | CYDMA_ELEMENT_WORD | CYDMA_INC_SRC_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

                /* Configure DMA descriptor. */
                ProtocovUSB_CyDmaSetConfiguration(channelNum, ProtocovUSB_DMA_DESCR1, ProtocovUSB_DMA_COMMON_CFG  |
                                                        CYDMA_BYTE | CYDMA_ELEMENT_WORD | CYDMA_INC_SRC_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

                /* Enable interrupt from DMA channel. */
                ProtocovUSB_CyDmaSetInterruptMask(channelNum);
                ...
                (void) CyDmaChDisable(ProtocovUSB_DmaChan[epNumber]);
                ...
                #if (ProtocovUSB_EP_DMA_AUTO_OPT == 0u)
                    ProtocovUSB_inLength[epNumber] = length;
                    ProtocovUSB_inDataPointer[epNumber] = pData;

                    /* Configure DMA to send data only for first burst */
                    (void) CyDmaTdSetConfiguration(ProtocovUSB_DmaTd[epNumber],
                        (length > ProtocovUSB_DMA_BYTES_PER_BURST) ? ProtocovUSB_DMA_BYTES_PER_BURST : length,
                        ProtocovUSB_DmaNextTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR);
                    (void) CyDmaTdSetAddress(ProtocovUSB_DmaTd[epNumber], LO16((uint32) pData),
                                                                               LO16((uint32) &ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].rwDr));
                    ...
                    */
                    (void) CyDmaTdSetConfiguration(ProtocovUSB_DmaNextTd[epNumber], 1u,
                                                   ProtocovUSB_DmaNextTd[epNumber],
                                                   ProtocovUSB_epX_TD_TERMOUT_EN[epNumber]);

                    /* Configure DmaNextTd to clear Data Ready status. */
                    (void) CyDmaTdSetAddress(ProtocovUSB_DmaNextTd[epNumber], LO16((uint32) &clearInDataRdyStatus),
                                                                                   LO16((uint32) &ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].epCfg));
                #else
                    /* Configure DMA to send all data. */
                    (void) CyDmaTdSetConfiguration(ProtocovUSB_DmaTd[epNumber], length,
                                                   ProtocovUSB_DmaTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR);
                    (void) CyDmaTdSetAddress(ProtocovUSB_DmaTd[epNumber], LO16((uint32) pData),
                                                                               LO16((uint32) &ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].rwDr));
                #endif /* (ProtocovUSB_EP_DMA_AUTO_OPT == 0u) */

                /* Clear any potential pending DMA requests before starting DMA channel to transfer data. */
                (void) CyDmaClearPendingDrq(ProtocovUSB_DmaChan[epNumber]);
                /* Enable DMA. */
                (void) CyDmaChSetInitialTd(ProtocovUSB_DmaChan[epNumber], ProtocovUSB_DmaTd[epNumber]);
                (void) CyDmaChEnable(ProtocovUSB_DmaChan[epNumber], 1u);
            #endif /* (CY_PSOC4) */
            ...
            else
            {
                ...
                if (length > 0u)
                {
                    ...
                    ProtocovUSB_DmaEpBurstCnt[epNumber] = (uint8) (length / ProtocovUSB_DMA_BYTES_PER_BURST);

                    /* Get number of elements in the last burst. */
                    ProtocovUSB_DmaEpLastBurstEl[epNumber] = (uint8) (length % ProtocovUSB_DMA_BYTES_PER_BURST);

                    /* Get total number of bursts. */
                    ProtocovUSB_DmaEpBurstCnt[epNumber] += (0u != ProtocovUSB_DmaEpLastBurstEl[epNumber]) ? 1u : 0u;

                    /* Adjust number of data elements transferred in last burst. */
                    ProtocovUSB_DmaEpLastBurstEl[epNumber] = (0u != ProtocovUSB_DmaEpLastBurstEl[epNumber]) ?
                                                                          (ProtocovUSB_DmaEpLastBurstEl[epNumber] - 1u) :
                                                                          (ProtocovUSB_DMA_BYTES_PER_BURST - 1u);

                    /* Get number of data elements to transfer for descriptor 0 and 1. */
                    lengthDescr0 = (1u == ProtocovUSB_DmaEpBurstCnt[epNumber]) ? ProtocovUSB_DmaEpLastBurstEl[epNumber] : (ProtocovUSB_DMA_BYTES_PER_BURST - 1u);
                    lengthDescr1 = (2u == ProtocovUSB_DmaEpBurstCnt[epNumber]) ? ProtocovUSB_DmaEpLastBurstEl[epNumber] : (ProtocovUSB_DMA_BYTES_PER_BURST - 1u);


                    /* Mark which descriptor is last one to execute. */
                    ProtocovUSB_DmaEpLastBurstEl[epNumber] |= (0u != (ProtocovUSB_DmaEpBurstCnt[epNumber] & 0x1u)) ?
                                                                            ProtocovUSB_DMA_DESCR0_MASK : ProtocovUSB_DMA_DESCR1_MASK;

                    /* Restore DMA settings for current transfer. */
                    ProtocovUSB_CyDmaChDisable(channelNum);

                    /* Restore destination address for input endpoint. */
                    ProtocovUSB_CyDmaSetSrcAddress(channelNum, ProtocovUSB_DMA_DESCR0, (void*) ((uint32) ProtocovUSB_DmaEpBufferAddrBackup[epNumber]));
                    ProtocovUSB_CyDmaSetSrcAddress(channelNum, ProtocovUSB_DMA_DESCR1, (void*) ((uint32) ProtocovUSB_DmaEpBufferAddrBackup[epNumber] +
                                                                                                                   ProtocovUSB_DMA_BYTES_PER_BURST));

                    /* Set number of elements to transfer. */
                    ProtocovUSB_CyDmaSetNumDataElements(channelNum, ProtocovUSB_DMA_DESCR0, lengthDescr0);
                    ProtocovUSB_CyDmaSetNumDataElements(channelNum, ProtocovUSB_DMA_DESCR1, lengthDescr1);
                    ...
                    if (ProtocovUSB_DmaEpBurstCnt[epNumber] > 1u)
                    {
                    ...
                    ProtocovUSB_DmaEpBurstCnt[epNumber] = ProtocovUSB_DMA_GET_BURST_CNT(ProtocovUSB_DmaEpBurstCnt[epNumber]);

                    /* Enable DMA channel: configuration complete. */
                    ProtocovUSB_CyDmaChEnable(channelNum);

                #elif (ProtocovUSB_EP_DMA_AUTO_OPT == 0u)
                    ProtocovUSB_inLength[epNumber]  = length;
                    ProtocovUSB_inBufFull[epNumber] = 0u;

                    (void) CyDmaChDisable(ProtocovUSB_DmaChan[epNumber]);
                    /* Configure DMA to send data only for first burst. */
                    (void) CyDmaTdSetConfiguration(
                        ProtocovUSB_DmaTd[epNumber], (length > ProtocovUSB_DMA_BYTES_PER_BURST) ?
                        ProtocovUSB_DMA_BYTES_PER_BURST : length,
                        ProtocovUSB_DmaNextTd[epNumber], TD_TERMIN_EN | TD_INC_SRC_ADR );
                    (void) CyDmaTdSetAddress(ProtocovUSB_DmaTd[epNumber], LO16((uint32)  ProtocovUSB_inDataPointer[epNumber]),
                                                                               LO16((uint32) &ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].rwDr));
                    /* Clear Any potential pending DMA requests before starting DMA channel to transfer data. */
                    (void) CyDmaClearPendingDrq(ProtocovUSB_DmaChan[epNumber]);
                    /* Enable DMA. */
                ...
                #if !defined (ProtocovUSB_MANUAL_IN_EP_ARM)
                    /* Set IN data ready to generate DMA request to load data into endpoint buffer. */
                    ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].epCfg |= ProtocovUSB_ARB_EPX_CFG_IN_DATA_RDY;
                #endif  /* (ProtocovUSB_MANUAL_IN_EP_ARM) */
        ...
        #endif /* (ProtocovUSB_EP_MANAGEMENT_DMA_AUTO) */
    #endif /* (ProtocovUSB_EP_MANAGEMENT_MANUAL) */
    }
}


/*******************************************************************************
* Function Name: ProtocovUSB_ReadOutEP
****************************************************************************//**
*
*   This function performs different functionality depending on the Component’s
...
*   transfer after calling the ProtocovUSB_ReadOutEP() API. For example,
*   by checking EPstate:
*
...
{
    if ((pData != NULL) && (epNumber > ProtocovUSB_EP0) && (epNumber < ProtocovUSB_MAX_EP))
    {
    #if (!ProtocovUSB_EP_MANAGEMENT_DMA_AUTO)
        /* Adjust requested length to available data. */
        length = (length > ProtocovUSB_GetEPCount(epNumber)) ? ProtocovUSB_GetEPCount(epNumber) : length;
    #endif /* (!ProtocovUSB_EP_MANAGEMENT_DMA_AUTO) */
    ...
    #if (ProtocovUSB_EP_MANAGEMENT_MANUAL)
        {
            ...
            for (i = 0u; i < length; ++i)
            {
                pData[i] = (uint8) ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].rwDr;
            }
        }

        /* Arm OUT endpoint after data has been copied from endpoint buffer. */
        ProtocovUSB_EnableOutEP(epNumber);
    #else
        ...
        #if (CY_PSOC4)
        {
            ...
            /* Configure source and destination. */
            ProtocovUSB_CyDmaSetSrcAddress(channelNum, ProtocovUSB_DMA_DESCR0, (void*) &ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].rwDr);
            ProtocovUSB_CyDmaSetDstAddress(channelNum, ProtocovUSB_DMA_DESCR0, (void*) pData);

            /* Configure DMA descriptor. */
            ProtocovUSB_CyDmaSetConfiguration(channelNum, ProtocovUSB_DMA_DESCR0, ProtocovUSB_DMA_COMMON_CFG | (uint16)(length - 1u) |
            ...
            /* Configure DMA to transfer data. */
            (void) CyDmaChDisable(ProtocovUSB_DmaChan[epNumber]);
            (void) CyDmaTdSetConfiguration(ProtocovUSB_DmaTd[epNumber], length, CY_DMA_DISABLE_TD, TD_TERMIN_EN | TD_INC_DST_ADR);
            (void) CyDmaTdSetAddress(ProtocovUSB_DmaTd[epNumber], LO16((uint32) &ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].rwDr), LO16((uint32)pData));
        ...
        #if (CY_PSOC4)
        {
            ...
            ProtocovUSB_DmaEpLastBurstEl[epNumber] |= (ProtocovUSB_DmaEpBurstCnt[epNumber] > 2u) ? ProtocovUSB_DMA_DESCR_REVERT : 0u;
            ...
            ProtocovUSB_DmaEpBufferAddrBackup[epNumber] = (uint32) pData;
            ProtocovUSB_DmaEpBurstCntBackup[epNumber]   = ProtocovUSB_DmaEpBurstCnt[epNumber];
            ...
            /* Set destination address. */
            ProtocovUSB_CyDmaSetSrcAddress(channelNum, ProtocovUSB_DMA_DESCR0, (void*) &ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].rwDr);
            ProtocovUSB_CyDmaSetSrcAddress(channelNum, ProtocovUSB_DMA_DESCR1, (void*) &ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].rwDr);

            ProtocovUSB_CyDmaSetDstAddress(channelNum, ProtocovUSB_DMA_DESCR0, (void*) ((uint32) pData));
            ProtocovUSB_CyDmaSetDstAddress(channelNum, ProtocovUSB_DMA_DESCR1, (void*) ((uint32) pData + ProtocovUSB_DMA_BYTES_PER_BURST));

            /* Configure DMA descriptor. */
            ProtocovUSB_CyDmaSetConfiguration(channelNum, ProtocovUSB_DMA_DESCR0, ProtocovUSB_DMA_COMMON_CFG  | lengthDescr0 |
                                                    CYDMA_BYTE | CYDMA_WORD_ELEMENT | CYDMA_INC_DST_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);

            /* Configure DMA descriptor. */
            ProtocovUSB_CyDmaSetConfiguration(channelNum, ProtocovUSB_DMA_DESCR1, ProtocovUSB_DMA_COMMON_CFG  | lengthDescr1 |
                                                    CYDMA_BYTE | CYDMA_WORD_ELEMENT | CYDMA_INC_DST_ADDR | CYDMA_INVALIDATE | CYDMA_CHAIN);
            ...
            if (ProtocovUSB_DmaEpBurstCntBackup[epNumber] > 1u)
            {
                ProtocovUSB_CyDmaValidateDescriptor(channelNum, ProtocovUSB_DMA_DESCR1);
            }
        ...
        #else
            (void) CyDmaChDisable(ProtocovUSB_DmaChan[epNumber]);
            (void) CyDmaTdSetConfiguration(ProtocovUSB_DmaTd[epNumber], length,  ProtocovUSB_DmaTd[epNumber], TD_TERMIN_EN | TD_INC_DST_ADR);
            (void) CyDmaTdSetAddress(ProtocovUSB_DmaTd[epNumber], LO16((uint32) &ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].rwDr), LO16((uint32) pData));
...
/*******************************************************************************
* Function Name: ProtocovUSB_LoadInEP16
****************************************************************************//**
*
*  This function performs different functionality depending on the Component’s
*  configured Endpoint Buffer Management. This parameter is defined in
...
void ProtocovUSB_LoadInEP16(uint8 epNumber, const uint8 pData[], uint16 length)
{
    ...
    if ((epNumber > ProtocovUSB_EP0) && (epNumber < ProtocovUSB_MAX_EP))
    {
        ...
        /* Set count and data toggle. */
        ProtocovUSB_SIE_EP_BASE.sieEp[epNumber].epCnt0 = (uint32) HI8(length) | ProtocovUSB_EP[epNumber].epToggle;
        ProtocovUSB_SIE_EP_BASE.sieEp[epNumber].epCnt1 = (uint32) LO8(length);
        ...
        if (NULL != pData)
        {
            ...
            {
                ProtocovUSB_ARB_EP16_BASE.arbEp[epNumber].rwDr16 = dataBuf[i];
            }
        }
        ...
        ProtocovUSB_SIE_EP_BASE.sieEp[epNumber].epCr0 = ProtocovUSB_EP[epNumber].epMode;
            ...
            if ((pData != NULL) && (length > 0u))
            {
                ...
                /* Configure source and destination. */
                ProtocovUSB_CyDmaSetSrcAddress(channelNum, ProtocovUSB_DMA_DESCR0, (void*) pData);
                ProtocovUSB_CyDmaSetDstAddress(channelNum, ProtocovUSB_DMA_DESCR0, (void*) &ProtocovUSB_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
                ...
                /* Generate DMA request. */
                ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].epCfg |=  (uint32)  ProtocovUSB_ARB_EPX_CFG_DMA_REQ;
                ProtocovUSB_ARB_EP_BASE.arbEp[epNumber].epCfg &=  (uint32) ~ProtocovUSB_ARB_EPX_CFG_DMA_REQ;
            ...
            if (pData != NULL)
            {
                ...
                /* Set destination address. */
                ProtocovUSB_CyDmaSetDstAddress(channelNum, ProtocovUSB_DMA_DESCR0, (void*) &ProtocovUSB_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
                ProtocovUSB_CyDmaSetDstAddress(channelNum, ProtocovUSB_DMA_DESCR1, (void*) &ProtocovUSB_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            ...
            else
            {
                ...
                if (length > 0u)
                {
                    ...
                    /* Adjust number of data elements transferred in last burst. */
                    ProtocovUSB_DmaEpLastBurstEl[epNumber] = (0u != ProtocovUSB_DmaEpLastBurstEl[epNumber]) ?
                                                                          ((ProtocovUSB_DmaEpLastBurstEl[epNumber] >> 1u) - 1u) :
                                                                           (ProtocovUSB_DMA_HALFWORDS_PER_BURST - 1u);

                    /* Get number of data elements to transfer for descriptor 0 and 1. */
                    lengthDescr0 = (1u == ProtocovUSB_DmaEpBurstCnt[epNumber]) ? ProtocovUSB_DmaEpLastBurstEl[epNumber] : (ProtocovUSB_DMA_HALFWORDS_PER_BURST - 1u);
                    lengthDescr1 = (2u == ProtocovUSB_DmaEpBurstCnt[epNumber]) ? ProtocovUSB_DmaEpLastBurstEl[epNumber] : (ProtocovUSB_DMA_HALFWORDS_PER_BURST - 1u);
...
/*******************************************************************************
* Function Name: ProtocovUSB_ReadOutEP16
****************************************************************************//**
*
*   This function performs different functionality depending on the Component’s
*   configured Endpoint Buffer Management. This parameter is defined in the
...
uint16 ProtocovUSB_ReadOutEP16(uint8 epNumber, uint8 pData[], uint16 length)
{
    ...
    adjLength =  length + ((uint32)length & 1u);
    ...
    #if (ProtocovUSB_EP_MANAGEMENT_MANUAL)
        {
            ...
            {
                dataBuf[i] = (uint16) ProtocovUSB_ARB_EP16_BASE.arbEp[epNumber].rwDr16;
            }
        }
        ...
        #if (ProtocovUSB_EP_MANAGEMENT_DMA_MANUAL)
        {
            ...
            /* Configure source and destination. */
            ProtocovUSB_CyDmaSetSrcAddress(channelNum, ProtocovUSB_DMA_DESCR0, (void*) &ProtocovUSB_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            ProtocovUSB_CyDmaSetDstAddress(channelNum, ProtocovUSB_DMA_DESCR0, (void*) pData);

            /* Configure DMA descriptor. */
            ProtocovUSB_CyDmaSetConfiguration(channelNum, ProtocovUSB_DMA_DESCR0, ProtocovUSB_DMA_COMMON_CFG | (uint16)((adjLength >> 1u) - 1u) |
        ...
        #if (ProtocovUSB_EP_MANAGEMENT_DMA_AUTO)
        {
            ...
            /* Get number of full bursts. */
            ProtocovUSB_DmaEpBurstCnt[epNumber] = (uint8) (adjLength / ProtocovUSB_DMA_BYTES_PER_BURST);

            /* Get number of elements in last burst. */
            ProtocovUSB_DmaEpLastBurstEl[epNumber] = (uint8) (adjLength % ProtocovUSB_DMA_BYTES_PER_BURST);
            ...
            ProtocovUSB_DmaEpLastBurstEl[epNumber] |= ProtocovUSB_DMA_DESCR_16BITS;
            ...
            /* Set destination address. */
            ProtocovUSB_CyDmaSetSrcAddress(channelNum, ProtocovUSB_DMA_DESCR0, (void*) &ProtocovUSB_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
            ProtocovUSB_CyDmaSetSrcAddress(channelNum, ProtocovUSB_DMA_DESCR1, (void*) &ProtocovUSB_ARB_EP16_BASE.arbEp[epNumber].rwDr16);
...
/*******************************************************************************
* Function Name: ProtocovUSB_EnableOutEP
****************************************************************************//**
*
...
* \globalvars
*
*  \ref ProtocovUSB_EP[epNumber].apiEpState - set to NO_EVENT_PENDING
*
* \reentrant
*  No.
*
...
/*******************************************************************************
* Function Name: ProtocovUSB_DisableOutEP
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: ProtocovUSB_Force
****************************************************************************//**
*
...
*    State                      |Description
*    ---------------------------|----------------------------------------------
*    ProtocovUSB_FORCE_J   | Force a J State onto the D+/D– lines
*    ProtocovUSB_FORCE_K   | Force a K State onto the D+/D– lines
*    ProtocovUSB_FORCE_SE0 | Force a Single Ended 0 onto the D+/D– lines
*    ProtocovUSB_FORCE_NONE| Return bus to SIE control
*
*
*******************************************************************************/
void ProtocovUSB_Force(uint8 bState) 
{
    ...
    */
    ProtocovUSB_USBIO_CR0_REG = bState;
}


/*******************************************************************************
* Function Name: ProtocovUSB_GetEPAckState
****************************************************************************//**
*
...
uint8 ProtocovUSB_GetEPAckState(uint8 epNumber) 
{
    ...
    if ((epNumber > ProtocovUSB_EP0) && (epNumber < ProtocovUSB_MAX_EP))
    {
        cr = ProtocovUSB_SIE_EP_BASE.sieEp[epNumber].epCr0 & ProtocovUSB_MODE_ACKD;
    }
...
/*******************************************************************************
* Function Name: ProtocovUSB_SetPowerStatus
****************************************************************************//**
*
...
*  --------------------------------------------|---------------------------
*  ProtocovUSB_DEVICE_STATUS_BUS_POWERED  | Set the device to bus powered
*  ProtocovUSB_DEVICE_STATUS_SELF_POWERED | Set the device to self powered
*
* \globalvars
*
*  \ref ProtocovUSB_deviceStatus - set power status
*
* \reentrant
*  No.
*
...
{
    if (powerStatus != ProtocovUSB_DEVICE_STATUS_BUS_POWERED)
    {
    ...
    /***************************************************************************
    * Function Name: ProtocovUSB_VBusPresent
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return ((0u != (ProtocovUSB_VBUS_STATUS_REG & ProtocovUSB_VBUS_VALID)) ? (uint8) 1u : (uint8) 0u);
    }
#endif /* (ProtocovUSB_VBUS_MONITORING_ENABLE) */


/*******************************************************************************
* Function Name: ProtocovUSB_RWUEnabled
****************************************************************************//**
*
...
*
* \globalvars
*  ProtocovUSB_deviceStatus - checked to determine remote status
*
*******************************************************************************/
uint8 ProtocovUSB_RWUEnabled(void) 
{
    uint8 result = ProtocovUSB_FALSE;

    if (0u != (ProtocovUSB_deviceStatus & ProtocovUSB_DEVICE_STATUS_REMOTE_WAKEUP))
    {
        result = ProtocovUSB_TRUE;
    }
...
/*******************************************************************************
* Function Name: ProtocovUSB_GetDeviceAddress
****************************************************************************//**
*
...
{
    return (uint8)(ProtocovUSB_CR0_REG & ProtocovUSB_CR0_DEVICE_ADDRESS_MASK);
}


/*******************************************************************************
* Function Name: ProtocovUSB_EnableSofInt
****************************************************************************//**
*
...
void ProtocovUSB_EnableSofInt(void) 
{
    ...
    ProtocovUSB_INTR_SIE_MASK_REG |= (uint32) ProtocovUSB_INTR_SIE_SOF_INTR;
#else
...
/*******************************************************************************
* Function Name: ProtocovUSB_DisableSofInt
****************************************************************************//**
*
...
void ProtocovUSB_DisableSofInt(void) 
{
    ...
    /***************************************************************************
    * Function Name: ProtocovUSB_DetectPortType
    ************************************************************************//**
    *
    ...
    *   Return Value                      |Description
    *   ----------------------------------|-------------------------------------
    *   ProtocovUSB_BCD_PORT_SDP     | Standard downstream port detected
    *   ProtocovUSB_BCD_PORT_CDP     | Charging downstream port detected
    *   ProtocovUSB_BCD_PORT_DCP     | Dedicated charging port detected
    *   ProtocovUSB_BCD_PORT_UNKNOWN | Unable to detect charging port type (proprietary charger type)
    *   ProtocovUSB_BCD_PORT_ERR     | Error condition in detection process
    *
    *
    * \sideeffects
    *
    ...
    ***************************************************************************/
    uint8 ProtocovUSB_Bcd_DetectPortType(void)
    {
        ...
        uint8 result = ProtocovUSB_BCD_PORT_UNKNOWN;
        ...
        */
        bkPwrCtrl = ProtocovUSB_POWER_CTRL_REG;
        ProtocovUSB_POWER_CTRL_REG = ProtocovUSB_DEFAULT_POWER_CTRL_VBUS\
                            & (~ProtocovUSB_POWER_CTRL_ENABLE_VBUS_PULLDOWN)\
                            & (~ProtocovUSB_POWER_CTRL_ENABLE_DM_PULLDOWN);


        /* Enable PHY detector and single-ended and differential receivers.
         * Enable charger detection.  */
        ProtocovUSB_POWER_CTRL_REG |= ProtocovUSB_DEFAULT_POWER_CTRL_PHY\
                                         | ProtocovUSB_POWER_CTRL_ENABLE_CHGDET;
        ...
        */
        #if defined (ProtocovUSB_NO_DCD)
        CyDelay(ProtocovUSB_BCD_TIMEOUT);
        #else
        /* DCD implementation:*/

        {
            uint16 timeout = ProtocovUSB_BCD_TIMEOUT;
            uint8 connectionApproved = 0u;
            uint8 connected = 0u;

            /*   BCD spec 1.2: Turns on Idp_src and D- pull-down resistor */
            ProtocovUSB_POWER_CTRL_REG |= ProtocovUSB_POWER_CTRL_ENABLE_DM_PULLDOWN;
            ProtocovUSB_CHGDET_CTRL_REG |= ProtocovUSB_CHGDET_CTRL_DCD_SRC_EN;

            /* BCD spec 1.2: Waits for D+ to be low for a time of Tdcd_dbnc*/
            while ((0u != timeout) && (0u == connectionApproved))
            {
                if (0u == (ProtocovUSB_USBIO_CR1_REG & ProtocovUSB_USBIO_CR1_DP0))
                {
                    connected++;
                }
                else
                ...
                connectionApproved = (ProtocovUSB_BCD_TDCD_DBNC < connected) ? 1u:0u;
                CyDelay(1u);
                timeout--;
            }
        ...
        #endif /*(ProtocovUSB_NO_DCD)*/

        /* Primary detection: enable VDP_SRC on D+ and IDM_SINK on D-. */
        ProtocovUSB_CHGDET_CTRL_REG = ProtocovUSB_CHGDET_CTRL_PRIMARY;
        CyDelay(ProtocovUSB_BCD_PRIMARY_WAIT);
        cr1RegVal = ProtocovUSB_USBIO_CR1_REG;

        /* Check is it SDP or DCP/CDP, read comparator 2 output. */
        if (0u == (ProtocovUSB_CHGDET_CTRL_REG & ProtocovUSB_CHGDET_CTRL_COMP_OUT))
        {
            /* Check status of D- line. */
            if (0u == (cr1RegVal & ProtocovUSB_USBIO_CR1_DM0))
            {
                result = ProtocovUSB_BCD_PORT_SDP;
            }
            else
            {
                 ...
                 * detection.
                */
                result = ProtocovUSB_BCD_PORT_ERR;
            }
        }
        else
        {
        ...
        {
            ProtocovUSB_CHGDET_CTRL_REG = ProtocovUSB_CHGDET_CTRL_DEFAULT \
                                                | ProtocovUSB_CHGDET_CTRL_SECONDARY;
            CyDelay(ProtocovUSB_BCD_SECONDARY_WAIT);
            cr1RegVal = ProtocovUSB_USBIO_CR1_REG;
            ...
            {
                /* Check status of D+ line. */
                if (0u == (cr1RegVal & ProtocovUSB_USBIO_CR1_DP0))
                {
                    result = ProtocovUSB_BCD_PORT_CDP;
                }
                else
                {
                    /* ERROR: such combination is impossible. Abort charger
            ...
            else
            {
                ...
                if (0u == (cr1RegVal & ProtocovUSB_USBIO_CR1_DP0))
                {
                    result = ProtocovUSB_BCD_PORT_DCP;
                }
                else
                {
                    ...
                    */
                    result = ProtocovUSB_BCD_PORT_UNKNOWN;
                }
            }
        }

        /* Restore CHGDET_CTRL. */
        ProtocovUSB_CHGDET_CTRL_REG = 0u;

        /*Revert registers back*/
        ProtocovUSB_POWER_CTRL_REG = bkPwrCtrl;
        ProtocovUSB_USBIO_CR1_REG |= (uint32) ProtocovUSB_USBIO_CR1_IOMODE;
        ProtocovUSB_USB_CLK_EN_REG = ~ProtocovUSB_USB_CLK_CSR_CLK_EN;
...
#if (ProtocovUSB_LPM_ACTIVE)
    /***************************************************************************
    * Function Name: ProtocovUSB_Lpm_GetBeslValue
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return (uint32) (ProtocovUSB_LPM_STAT_REG & ProtocovUSB_LPM_STAT_LPM_BESL_MASK);
    }


    /***************************************************************************
    * Function Name: ProtocovUSB_Lpm_RemoteWakeUpAllowed
    ************************************************************************//**
    *
    ...
    ***************************************************************************/
    ...
    {
        return (uint32) (ProtocovUSB_LPM_STAT_REG & ProtocovUSB_LPM_STAT_LPM_REMOTE_WAKE);
    }


    /***************************************************************************
    * Function Name: ProtocovUSB_Lpm_SetResponse
    ************************************************************************//**
    *
    ...
    *       - ProtocovUSB_LPM_REQ_ACK - next LPM request will be
    *                                           responded with ACK
    *       - ProtocovUSB_LPM_REQ_NACK - next LPM request will be
    *                                           responded with NACK
    *       - ProtocovUSB_LPM_REQ_NYET - next LPM request will be
    *                                           responded with NYET
    *
    ***************************************************************************/
    void ProtocovUSB_Lpm_SetResponse(uint32 response)
    {
        uint32 lpmCtrl = ProtocovUSB_LPM_CTRL_REG & (uint32) ~ProtocovUSB_LPM_CTRL_ACK_NYET_MASK;

        ProtocovUSB_LPM_CTRL_REG = lpmCtrl | ((uint32) response & ProtocovUSB_LPM_CTRL_ACK_NYET_MASK);
    }


    /***************************************************************************
    * Function Name: ProtocovUSB_Lpm_GetResponse
    ************************************************************************//**
    *
    ...
    *       - ProtocovUSB_LPM_REQ_ACK - next LPM request will be responded
    *                                        with ACK
    *       - ProtocovUSB_LPM_REQ_NACK - next LPM request will be responded
    *                                        with NACK
    *       - ProtocovUSB_LPM_REQ_NYET - next LPM request will be responded
    *                                        with NYET
    *
    ***************************************************************************/
    uint32 ProtocovUSB_Lpm_GetResponse(void)
    {
        ...
        return  ((uint32) ProtocovUSB_LPM_CTRL_REG & (uint32)ProtocovUSB_LPM_CTRL_ACK_NYET_MASK);
    }
