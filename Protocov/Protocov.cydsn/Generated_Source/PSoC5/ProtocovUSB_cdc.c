 ...
 * ProtocovUSB_GetDTERate(), ProtocovUSB_GetCharFormat(),
 * ProtocovUSB_GetParityType(), and ProtocovUSB_GetDataBits() APIs.
...
volatile uint8 ProtocovUSB_linesCoding[ProtocovUSB_MAX_MULTI_COM_NUM][ProtocovUSB_LINE_CODING_SIZE] =
{
    /*COM Port 1*/
    {
...
/**Used as a flag for the ProtocovUSB_IsLineChanged() API, to inform it that the
 ...
 * support. In case of 1 COM port, data is in 0 element.*/
volatile uint8  ProtocovUSB_linesChanged[ProtocovUSB_MAX_MULTI_COM_NUM];
 ...
 * ProtocovUSB_GetLineControl() API. It is an array of 2 elements for COM 
 * port 1 and COM port 2 for MultiCOM port support. In case of 1 COM port, data 
 * is in 0 element.*/
volatile uint16 ProtocovUSB_linesControlBitmap[ProtocovUSB_MAX_MULTI_COM_NUM];
/** Contains the 16-bit serial state value that was sent using the 
 * \ref ProtocovUSB_SendSerialState() API. . It is an array of 2 elements 
 * for COM port 1 and COM port 2 for MultiCOM port support. In case of 1 COM 
 * port, data is in 0 element.*/
volatile uint16 ProtocovUSB_serialStateBitmap[ProtocovUSB_MAX_MULTI_COM_NUM];
...
volatile uint8  ProtocovUSB_cdcDataInEp[ProtocovUSB_MAX_MULTI_COM_NUM];
 ...
 * port 2 for MultiCOM port support. In case of 1 COM port, data is in 0 element.*/
volatile uint8  ProtocovUSB_cdcDataOutEp[ProtocovUSB_MAX_MULTI_COM_NUM];
...
volatile uint8  ProtocovUSB_cdcCommInInterruptEp[ProtocovUSB_MAX_MULTI_COM_NUM];

/*PRIVATE*/

#define ProtocovUSB_CDC_IN_EP      (0u)
#define ProtocovUSB_CDC_OUT_EP     (1u)
#define ProtocovUSB_CDC_NOTE_EP    (2u)

#define ProtocovUSB_CDC_EP_MASK    (0x01u)

#define ProtocovUSB_GET_EP_COM_NUM(cdcComNums, epType) (((cdcComNums) >> (epType)) & ProtocovUSB_CDC_EP_MASK)
...
***************************************/
    ...
    static uint16 ProtocovUSB_StrLen(const char8 string[]) ;
    static t_ProtocovUSB_cdc_notification ProtocovUSB_serialStateNotification =
    {

        ProtocovUSB_SERIAL_STATE_REQUEST_TYPE, /* bRequestType    */
        ProtocovUSB_SERIAL_STATE,              /* bNotification   */
        0u,                                         /* wValue          */
        0u,                                         /* wValueMSB       */
        0u,                                         /* wIndex          */
        0u,                                         /* wIndexMSB       */
        ProtocovUSB_SERIAL_STATE_LENGTH,       /* wLength         */
        0u,                                         /* wLengthMSB      */
        0u,                                         /* wSerialState    */
        0u,                                         /* wSerialStateMSB */
    };
    static uint8 ProtocovUSB_activeCom = 0u;
#endif /* (ProtocovUSB_ENABLE_CDC_CLASS_API != 0u) */
...
/*******************************************************************************
* Function Name: ProtocovUSB_DispatchCDCClassRqst
****************************************************************************//**
*
...
*
* \globalvars
*   ProtocovUSB_linesCoding: Contains the current line coding structure.
...
*   ProtocovUSB_linesControlBitmap: Contains the current control signal
*     bitmap. It is set by the Host using SET_CONTROL_LINE request and returned
*     to the user code by the USBFS_GetLineControl() API.
*   ProtocovUSB_linesChanged: This variable is used as a flag for the
...
uint8 ProtocovUSB_DispatchCDCClassRqst(void) 
{
    ...
    comPort = ProtocovUSB_GetInterfaceComPort((uint8)ProtocovUSB_wIndexLoReg);
    ...
    if (0u != (ProtocovUSB_bmRequestTypeReg & ProtocovUSB_RQST_DIR_D2H))
    {
        ...
        {
            case ProtocovUSB_CDC_GET_LINE_CODING:
                ProtocovUSB_currentTD.count = ProtocovUSB_LINE_CODING_SIZE;
                ProtocovUSB_currentTD.pData = ProtocovUSB_linesCoding[comPort];
            ...
            #ifdef ProtocovUSB_DISPATCH_CDC_CLASS_CDC_READ_REQUESTS_CALLBACK
                requestHandled = ProtocovUSB_DispatchCDCClass_CDC_READ_REQUESTS_Callback();
            #endif /* (ProtocovUSB_DISPATCH_CDC_CLASS_CDC_READ_REQUESTS_CALLBACK) */
                break;
        }
    }
    else
    {
        ...
        switch (ProtocovUSB_bRequestReg)
        {
            case ProtocovUSB_CDC_SET_LINE_CODING:
                ProtocovUSB_currentTD.count  = ProtocovUSB_LINE_CODING_SIZE;
                ProtocovUSB_currentTD.pData  = ProtocovUSB_linesCoding[comPort];
                ProtocovUSB_linesChanged[comPort] |= ProtocovUSB_LINE_CODING_CHANGED;

                requestHandled = ProtocovUSB_InitControlWrite();
                break;

            case ProtocovUSB_CDC_SET_CONTROL_LINE_STATE:
                ProtocovUSB_linesControlBitmap[comPort] = (uint8) ProtocovUSB_wValueLoReg;
                ProtocovUSB_linesChanged[comPort]      |= ProtocovUSB_LINE_CONTROL_CHANGED;
            ...
            default:
                /* Do not handle this request unless callback is defined. */
            #ifdef ProtocovUSB_DISPATCH_CDC_CLASS_CDC_WRITE_REQUESTS_CALLBACK
                requestHandled = ProtocovUSB_DispatchCDCClass_CDC_WRITE_REQUESTS_Callback();
            #endif /* (ProtocovUSB_DISPATCH_CDC_CLASS_CDC_WRITE_REQUESTS_CALLBACK) */
                break;
        }
    }
...
/***************************************************************************
* Function Name: ProtocovUSB_GetInterfaceComPort
************************************************************************//**
*   \internal
...
uint8 ProtocovUSB_GetInterfaceComPort(uint8 interface) 
{
    ...
    while (comPort < ProtocovUSB_MAX_MULTI_COM_NUM)
    {
        notEp = ProtocovUSB_cdcCommInInterruptEp[comPort];

        if (ProtocovUSB_EP[notEp].interface == interface)
        {
            res = comPort;
            comPort = ProtocovUSB_MAX_MULTI_COM_NUM;
        }
...
#if (ProtocovUSB_ENABLE_CDC_CLASS_API != 0u)
/***************************************************************************
* Function Name: ProtocovUSB_CDC_Init
************************************************************************//**
*
...
*   ProtocovUSB_SUCCESS   CDC interface was initialized correctly
*   ProtocovUSB_FAILURE   CDC interface was not initialized
*
* \globalvars
*   ProtocovUSB_linesChanged: Initialized to zero.
*   ProtocovUSB_cdcDataOutEp: Used as an OUT endpoint number.
*
* \reentrant
*  No.
*
...
uint8 ProtocovUSB_CDC_Init(void) 
{
    uint8 comPort;
    uint8 outEp;
    uint8 ret = ProtocovUSB_SUCCESS;

    ProtocovUSB_activeCom = 0u;
    ProtocovUSB_linesChanged[ProtocovUSB_COM_PORT1] = 0u;
    ProtocovUSB_linesChanged[ProtocovUSB_COM_PORT2] = 0u;

    for(comPort = 0u; comPort<ProtocovUSB_MAX_MULTI_COM_NUM; comPort++)
    {
        outEp = ProtocovUSB_cdcDataOutEp[comPort];
        if((0u != outEp) && (ProtocovUSB_MAX_EP > outEp))
        {
            ProtocovUSB_EnableOutEP(outEp);
        }

    }

    /* COM Port 1 should be correct to proceed. */
    if ((0u == ProtocovUSB_cdcDataInEp[ProtocovUSB_COM_PORT1]) \
            || (0u == ProtocovUSB_cdcDataOutEp[ProtocovUSB_COM_PORT1]) \
            || (0u ==  ProtocovUSB_cdcCommInInterruptEp[ProtocovUSB_COM_PORT1])
            || (ProtocovUSB_cdcDataInEp[ProtocovUSB_COM_PORT1] >= ProtocovUSB_MAX_EP)
            || (ProtocovUSB_cdcDataOutEp[ProtocovUSB_COM_PORT1] >= ProtocovUSB_MAX_EP)
            || (ProtocovUSB_cdcCommInInterruptEp[ProtocovUSB_COM_PORT1] >= ProtocovUSB_MAX_EP))
    {
        ret = ProtocovUSB_FAILURE;
    }
...
/*******************************************************************************
* Function Name: ProtocovUSB_PutData
****************************************************************************//**
*
*  This function sends a specified number of bytes from the location specified
*  by a pointer to the PC. The ProtocovUSB_CDCIsReady() function should be
...
*  identified. To send zero-length packet, use ProtocovUSB_PutData() API
...
* \globalvars
*
*   ProtocovUSB_cdcDataInEp: CDC IN endpoint number used for sending
*     data.
*
* \reentrant
*  No.
...
{
    uint8 epNumber = ProtocovUSB_cdcDataInEp[ProtocovUSB_activeCom];

    /* Limit length to maximum packet size for endpoint. */
    if (length > ProtocovUSB_EP[epNumber].bufferSize)
    {
        /* Caution: Data will be lost if length is greater than Max Packet Size. */
        length = ProtocovUSB_EP[epNumber].bufferSize;

        /* Halt CPU in debug mode */
        CYASSERT(0u != 0u);
    }

    ProtocovUSB_LoadInEP(epNumber, pData, length);
}


/*******************************************************************************
* Function Name: ProtocovUSB_StrLen
****************************************************************************//**
*
...
/***************************************************************************
* Function Name: ProtocovUSB_PutString
************************************************************************//**
*
...
*  The ProtocovUSB_CDCIsReady() function should be called before
*  sending data with a new call to ProtocovUSB_PutString(), to be sure
...
void ProtocovUSB_PutString(const char8 string[]) 
{
    ...
    strLength = ProtocovUSB_StrLen(string);

    do
    {
        /* Limit length to maximum packet size of endpoint. */
        sendLength = (strLength > ProtocovUSB_EP[epNumber].bufferSize) ?
                                  ProtocovUSB_EP[epNumber].bufferSize : strLength;

        /* Load IN endpoint and expose it to host. */
        ProtocovUSB_LoadInEP(epNumber, (const uint8 *)&string[bufIndex], sendLength);
        strLength -= sendLength;

        /* If more data are present to send or full packet was sent */
        if ((strLength > 0u) || (sendLength == ProtocovUSB_EP[epNumber].bufferSize))
        {
            bufIndex += sendLength;

            /* Wait until host read data from IN endpoint buffer. */
            while (ProtocovUSB_IN_BUFFER_FULL == ProtocovUSB_EP[epNumber].apiEpState)
            {
            }
            ...
            */
            if (0u == strLength)
            {
                ProtocovUSB_LoadInEP(epNumber, NULL, 0u);
            }
        }
    }
    while (strLength > 0u);
...
/***************************************************************************
* Function Name: ProtocovUSB_PutChar
************************************************************************//**
*
...
void ProtocovUSB_PutChar(char8 txDataByte) 
{
    ...
    ProtocovUSB_LoadInEP(ProtocovUSB_cdcDataInEp[ProtocovUSB_activeCom], &dataByte, 1u);
}


/*******************************************************************************
* Function Name: ProtocovUSB_PutCRLF
****************************************************************************//**
*
...
void ProtocovUSB_PutCRLF(void) 
{
    ...
    ProtocovUSB_LoadInEP(ProtocovUSB_cdcDataInEp[ProtocovUSB_activeCom], (const uint8 *)txData, 2u);
}


/*******************************************************************************
* Function Name: ProtocovUSB_GetCount
****************************************************************************//**
*
*  This function returns the number of bytes that were received from the PC.
*  The returned length value should be passed to ProtocovUSB_GetData() as
*  a parameter to read all received data. If all of the received data is not
*  read at one time by the ProtocovUSB_GetData() API, the unread data will
*  be lost.
*
* \return
...
*
* \globalvars
*   ProtocovUSB_cdcDataOutEp: CDC OUT endpoint number used.
*
*******************************************************************************/
uint16 ProtocovUSB_GetCount(void) 
{
    ...
    uint8  epNumber = ProtocovUSB_cdcDataOutEp[ProtocovUSB_activeCom];

    if (ProtocovUSB_OUT_BUFFER_FULL == ProtocovUSB_EP[epNumber].apiEpState)
    {
        bytesCount = ProtocovUSB_GetEPCount(epNumber);
    }
    else
    {
        bytesCount = 0u;
...
/*******************************************************************************
* Function Name: ProtocovUSB_DataIsReady
****************************************************************************//**
*
*  This function returns a non-zero value if the component received data or
*  received zero-length packet. The ProtocovUSB_GetAll() or
*  ProtocovUSB_GetData() API should be called to read data from the buffer
...
{
    return (ProtocovUSB_GetEPState(ProtocovUSB_cdcDataOutEp[ProtocovUSB_activeCom]));
}


/*******************************************************************************
* Function Name: ProtocovUSB_CDCIsReady
****************************************************************************//**
*
...
*  ProtocovUSB_PutData(),ProtocovUSB_PutString(),
*  ProtocovUSB_PutChar or ProtocovUSB_PutCRLF(),
...
*
* \globalvars
*   ProtocovUSB_cdcDataInEp: CDC IN endpoint number used.
*
*******************************************************************************/
uint8 ProtocovUSB_CDCIsReady(void) 
{
    return (ProtocovUSB_GetEPState(ProtocovUSB_cdcDataInEp[ProtocovUSB_activeCom]));
}


/***************************************************************************
* Function Name: ProtocovUSB_GetData
************************************************************************//**
*
...
*  The ProtocovUSB_DataIsReady() API should be called first, to be sure
*  that data is received from the host. If all received data will not be read at
*  once, the unread data will be lost. The ProtocovUSB_GetData() API should
...
uint16 ProtocovUSB_GetData(uint8* pData, uint16 length) 
{
    ...
    length = ProtocovUSB_ReadOutEP(epNumber, pData, length);

#if (ProtocovUSB_EP_MANAGEMENT_DMA_MANUAL)
    /* Wait until DMA complete transferring data from OUT endpoint buffer. */
    while (ProtocovUSB_OUT_BUFFER_FULL == ProtocovUSB_GetEPState(epNumber))
    {
    }
...
*
* \globalvars
*   - \ref ProtocovUSB_cdcDataOutEp: CDC OUT endpoint number used.
*   - \ref ProtocovUSB_EP[].bufferSize: EP max packet size is used as a
...
uint16 ProtocovUSB_GetAll(uint8* pData) 
{
    ...
    /* Read data from OUT endpoint buffer. */
    dataLength = ProtocovUSB_ReadOutEP(epNumber, pData, ProtocovUSB_EP[epNumber].bufferSize);
...
/***************************************************************************
* Function Name: ProtocovUSB_GetChar
************************************************************************//**
*
...
uint8 ProtocovUSB_GetChar(void) 
{
    ...
    (void) ProtocovUSB_ReadOutEP(epNumber, &rxData, 1u);
...
/*******************************************************************************
* Function Name: ProtocovUSB_IsLineChanged
****************************************************************************//**
*
...
*  device. The ProtocovUSB_GetDTERate(), ProtocovUSB_GetCharFormat()
*  or ProtocovUSB_GetParityType() or ProtocovUSB_GetDataBits() API
*  should be called to read data coding information.
*  The ProtocovUSB_GetLineControl() API should be called to read line
*  control information.
*
* \return
...
*
* \globalvars
*  - \ref ProtocovUSB_transferState: it is checked to be sure then OUT
*    data phase has been complete, and data written to the lineCoding or
*    Control Bitmap buffer.
*  - \ref ProtocovUSB_linesChanged: used as a flag to be aware that
...
uint8 ProtocovUSB_IsLineChanged(void) 
{
    ...
    if (ProtocovUSB_transferState == ProtocovUSB_TRANS_STATE_IDLE)
    {
        if (ProtocovUSB_linesChanged[ProtocovUSB_activeCom] != 0u)
        {
            state = ProtocovUSB_linesChanged[ProtocovUSB_activeCom];
            ProtocovUSB_linesChanged[ProtocovUSB_activeCom] = 0u;
        }
    }
...
/***************************************************************************
* Function Name: ProtocovUSB_GetDTERate
************************************************************************//**
*
...
*
* \globalvars
*  ProtocovUSB_linesCoding: First four bytes converted to uint32
...
uint32 ProtocovUSB_GetDTERate(void) 
{
    ...
    rate = ProtocovUSB_linesCoding[ProtocovUSB_activeCom][ProtocovUSB_LINE_CODING_RATE + 3u];
    rate = (rate << 8u) | ProtocovUSB_linesCoding[ProtocovUSB_activeCom][ProtocovUSB_LINE_CODING_RATE + 2u];
    rate = (rate << 8u) | ProtocovUSB_linesCoding[ProtocovUSB_activeCom][ProtocovUSB_LINE_CODING_RATE + 1u];
    rate = (rate << 8u) | ProtocovUSB_linesCoding[ProtocovUSB_activeCom][ProtocovUSB_LINE_CODING_RATE];
...
/*******************************************************************************
* Function Name: ProtocovUSB_GetCharFormat
****************************************************************************//**
*
...
*
* \globalvars
*  ProtocovUSB_linesCoding: used to get a parameter.
*
*******************************************************************************/
uint8 ProtocovUSB_GetCharFormat(void) 
{
    return (ProtocovUSB_linesCoding[ProtocovUSB_activeCom][ProtocovUSB_LINE_CODING_STOP_BITS]);
}


/*******************************************************************************
* Function Name: ProtocovUSB_GetParityType
****************************************************************************//**
*
...
{
    return (ProtocovUSB_linesCoding[ProtocovUSB_activeCom][ProtocovUSB_LINE_CODING_PARITY]);
}


/***************************************************************************
* Function Name: ProtocovUSB_GetDataBits
************************************************************************//**
*
...
{
    return (ProtocovUSB_linesCoding[ProtocovUSB_activeCom][ProtocovUSB_LINE_CODING_DATA_BITS]);
}


/***************************************************************************
* Function Name: ProtocovUSB_GetLineControl
************************************************************************//**
*
...
*
* \globalvars
*  ProtocovUSB_linesControlBitmap: used to get a parameter.
*
*******************************************************************************/
uint16 ProtocovUSB_GetLineControl(void) 
{
    return (ProtocovUSB_linesControlBitmap[ProtocovUSB_activeCom]);
}


/*******************************************************************************
* Function Name: ProtocovUSB_SendSerialState
****************************************************************************//**
*
...
*  ProtocovUSB_NotificationIsReady() API must be called to check if the
...
{
    uint8 epNumber = ProtocovUSB_cdcCommInInterruptEp[ProtocovUSB_activeCom];

    if(ProtocovUSB_SERIAL_STATE_SIZE <= ProtocovUSB_EP[epNumber].bufferSize)
    {
        /* Save current SERIAL_STATE bitmap. */
        ProtocovUSB_serialStateBitmap[ProtocovUSB_activeCom] = serialState;

        /* Add interface number */
        ProtocovUSB_serialStateNotification.wIndex = ProtocovUSB_EP[epNumber].interface;

        /*Form SERIAL_STATE data*/
        ProtocovUSB_serialStateNotification.wSerialState =    LO8(ProtocovUSB_serialStateBitmap[ProtocovUSB_activeCom]);
        ProtocovUSB_serialStateNotification.wSerialStateMSB = HI8(ProtocovUSB_serialStateBitmap[ProtocovUSB_activeCom]);

        ProtocovUSB_LoadInEP(epNumber, (uint8 *) &ProtocovUSB_serialStateNotification, sizeof(ProtocovUSB_serialStateNotification));
    }
}


/*******************************************************************************
* Function Name: ProtocovUSB_GetSerialState
****************************************************************************//**
*
...
{
    return ProtocovUSB_serialStateBitmap[ProtocovUSB_activeCom];
}


/*******************************************************************************
* Function Name: ProtocovUSB_NotificationIsReady
****************************************************************************//**
*
...
*  ProtocovUSB_SendSerialState() to ensure that any previous
...
{
    return (ProtocovUSB_EP[ProtocovUSB_cdcCommInInterruptEp[ProtocovUSB_activeCom]].apiEpState);
}


/*******************************************************************************
* Function Name: ProtocovUSB_SetComPort
****************************************************************************//**
*
...
{
    if ((ProtocovUSB_activeCom != comNumber) && \
            (comNumber < ProtocovUSB_MAX_MULTI_COM_NUM ))
    {
        ProtocovUSB_activeCom = comNumber;
    }
}


/*******************************************************************************
* Function Name: ProtocovUSB_GetComPort
****************************************************************************//**
*
...
/***************************************************************************
* Function Name: ProtocovUSB_Cdc_EpInit
************************************************************************//**
*
*  \internal
...
*   ProtocovUSB_cdcDataInEp[], ProtocovUSB_cdcCommInInterruptEp[],
*   ProtocovUSB_cdcDataOutEp[]
*
...
uint8 ProtocovUSB_Cdc_EpInit(const T_ProtocovUSB_EP_SETTINGS_BLOCK CYCODE *pEP, uint8 epNum, uint8 cdcComNums) 
{
    ...
    {
        if (epType != ProtocovUSB_EP_TYPE_INT)
        {
            ProtocovUSB_cdcDataInEp[ProtocovUSB_GET_EP_COM_NUM(cdcComNums, ProtocovUSB_CDC_IN_EP)] = epNum;
            cdcComNums |= (uint8)(ProtocovUSB_COM_PORT2 << ProtocovUSB_CDC_IN_EP);
        }
        else
        {

            ProtocovUSB_cdcCommInInterruptEp[ProtocovUSB_GET_EP_COM_NUM(cdcComNums, ProtocovUSB_CDC_NOTE_EP)] = epNum;
            cdcComNums |= (uint8)(ProtocovUSB_COM_PORT2 << ProtocovUSB_CDC_NOTE_EP);
        }
    }
    else
    {
        if (epType != ProtocovUSB_EP_TYPE_INT)
        {
            ProtocovUSB_cdcDataOutEp[ProtocovUSB_GET_EP_COM_NUM(cdcComNums, ProtocovUSB_CDC_OUT_EP)] = epNum;
            cdcComNums |= (uint8)(ProtocovUSB_COM_PORT2 << ProtocovUSB_CDC_OUT_EP);
        }
    }
    return (cdcComNums);
}
