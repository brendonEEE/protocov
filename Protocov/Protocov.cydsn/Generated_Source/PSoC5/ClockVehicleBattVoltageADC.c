/*******************************************************************************
* File Name: ClockVehicleBattVoltageADC.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: ClockVehicleBattVoltageADC_Start
********************************************************************************
*
* Summary:
...
void ClockVehicleBattVoltageADC_Start(void) 
{
    ...
    ClockVehicleBattVoltageADC_CLKEN |= ClockVehicleBattVoltageADC_CLKEN_MASK;
	ClockVehicleBattVoltageADC_CLKSTBY |= ClockVehicleBattVoltageADC_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ClockVehicleBattVoltageADC_Stop
********************************************************************************
*
* Summary:
...
void ClockVehicleBattVoltageADC_Stop(void) 
{
    ...
    ClockVehicleBattVoltageADC_CLKEN &= (uint8)(~ClockVehicleBattVoltageADC_CLKEN_MASK);
	ClockVehicleBattVoltageADC_CLKSTBY &= (uint8)(~ClockVehicleBattVoltageADC_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ClockVehicleBattVoltageADC_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ClockVehicleBattVoltageADC_CLKEN & ClockVehicleBattVoltageADC_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ClockVehicleBattVoltageADC_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ClockVehicleBattVoltageADC_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ClockVehicleBattVoltageADC__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ClockVehicleBattVoltageADC_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ClockVehicleBattVoltageADC_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ClockVehicleBattVoltageADC_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ClockVehicleBattVoltageADC_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ClockVehicleBattVoltageADC_GetSourceRegister();
    uint16 oldDivider = ClockVehicleBattVoltageADC_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ClockVehicleBattVoltageADC_CLKEN & ClockVehicleBattVoltageADC_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ClockVehicleBattVoltageADC_DIV_PTR, clkDivider);
                ClockVehicleBattVoltageADC_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ClockVehicleBattVoltageADC_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ClockVehicleBattVoltageADC_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ClockVehicleBattVoltageADC_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ClockVehicleBattVoltageADC_DIV_PTR, clkDivider);
				ClockVehicleBattVoltageADC_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: ClockVehicleBattVoltageADC_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ClockVehicleBattVoltageADC_DIV_PTR);
}


/*******************************************************************************
* Function Name: ClockVehicleBattVoltageADC_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ClockVehicleBattVoltageADC_MOD_SRC |= modeBitMask & (uint8)ClockVehicleBattVoltageADC_MODE_MASK;
}


/*******************************************************************************
* Function Name: ClockVehicleBattVoltageADC_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ClockVehicleBattVoltageADC_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ClockVehicleBattVoltageADC_MODE_MASK));
}


/*******************************************************************************
* Function Name: ClockVehicleBattVoltageADC_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ClockVehicleBattVoltageADC_MOD_SRC & (uint8)(ClockVehicleBattVoltageADC_MODE_MASK);
}


/*******************************************************************************
* Function Name: ClockVehicleBattVoltageADC_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ClockVehicleBattVoltageADC_GetDividerRegister();
    uint8 oldSrc = ClockVehicleBattVoltageADC_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ClockVehicleBattVoltageADC_MOD_SRC |= CYCLK_SSS;
        ClockVehicleBattVoltageADC_MOD_SRC =
            (ClockVehicleBattVoltageADC_MOD_SRC & (uint8)(~ClockVehicleBattVoltageADC_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ClockVehicleBattVoltageADC_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ClockVehicleBattVoltageADC_MOD_SRC & ClockVehicleBattVoltageADC_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ClockVehicleBattVoltageADC_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ClockVehicleBattVoltageADC_PHASE = clkPhase & ClockVehicleBattVoltageADC_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ClockVehicleBattVoltageADC_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ClockVehicleBattVoltageADC_PHASE & ClockVehicleBattVoltageADC_PHASE_MASK;
}
