/*******************************************************************************
* File Name: VehicleBattVoltageAdc_IntClock.c
* Version 2.20
*
*  Description:
...
#include "VehicleBattVoltageAdc_IntClock.h"
...
/*******************************************************************************
* Function Name: VehicleBattVoltageAdc_IntClock_Start
********************************************************************************
*
* Summary:
...
void VehicleBattVoltageAdc_IntClock_Start(void) 
{
    ...
    VehicleBattVoltageAdc_IntClock_CLKEN |= VehicleBattVoltageAdc_IntClock_CLKEN_MASK;
	VehicleBattVoltageAdc_IntClock_CLKSTBY |= VehicleBattVoltageAdc_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: VehicleBattVoltageAdc_IntClock_Stop
********************************************************************************
*
* Summary:
...
void VehicleBattVoltageAdc_IntClock_Stop(void) 
{
    ...
    VehicleBattVoltageAdc_IntClock_CLKEN &= (uint8)(~VehicleBattVoltageAdc_IntClock_CLKEN_MASK);
	VehicleBattVoltageAdc_IntClock_CLKSTBY &= (uint8)(~VehicleBattVoltageAdc_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: VehicleBattVoltageAdc_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((VehicleBattVoltageAdc_IntClock_CLKEN & VehicleBattVoltageAdc_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = VehicleBattVoltageAdc_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = VehicleBattVoltageAdc_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* VehicleBattVoltageAdc_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(VehicleBattVoltageAdc_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(VehicleBattVoltageAdc_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: VehicleBattVoltageAdc_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: VehicleBattVoltageAdc_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = VehicleBattVoltageAdc_IntClock_GetSourceRegister();
    uint16 oldDivider = VehicleBattVoltageAdc_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = VehicleBattVoltageAdc_IntClock_CLKEN & VehicleBattVoltageAdc_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(VehicleBattVoltageAdc_IntClock_DIV_PTR, clkDivider);
                VehicleBattVoltageAdc_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                VehicleBattVoltageAdc_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(VehicleBattVoltageAdc_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((VehicleBattVoltageAdc_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(VehicleBattVoltageAdc_IntClock_DIV_PTR, clkDivider);
				VehicleBattVoltageAdc_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: VehicleBattVoltageAdc_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(VehicleBattVoltageAdc_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: VehicleBattVoltageAdc_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    VehicleBattVoltageAdc_IntClock_MOD_SRC |= modeBitMask & (uint8)VehicleBattVoltageAdc_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: VehicleBattVoltageAdc_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    VehicleBattVoltageAdc_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(VehicleBattVoltageAdc_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: VehicleBattVoltageAdc_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return VehicleBattVoltageAdc_IntClock_MOD_SRC & (uint8)(VehicleBattVoltageAdc_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: VehicleBattVoltageAdc_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = VehicleBattVoltageAdc_IntClock_GetDividerRegister();
    uint8 oldSrc = VehicleBattVoltageAdc_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        VehicleBattVoltageAdc_IntClock_MOD_SRC |= CYCLK_SSS;
        VehicleBattVoltageAdc_IntClock_MOD_SRC =
            (VehicleBattVoltageAdc_IntClock_MOD_SRC & (uint8)(~VehicleBattVoltageAdc_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: VehicleBattVoltageAdc_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return VehicleBattVoltageAdc_IntClock_MOD_SRC & VehicleBattVoltageAdc_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: VehicleBattVoltageAdc_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    VehicleBattVoltageAdc_IntClock_PHASE = clkPhase & VehicleBattVoltageAdc_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: VehicleBattVoltageAdc_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return VehicleBattVoltageAdc_IntClock_PHASE & VehicleBattVoltageAdc_IntClock_PHASE_MASK;
}
