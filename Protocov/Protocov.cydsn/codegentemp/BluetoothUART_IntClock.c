/*******************************************************************************
* File Name: BluetoothUART_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: BluetoothUART_IntClock_Start
********************************************************************************
*
* Summary:
...
void BluetoothUART_IntClock_Start(void) 
{
    ...
    BluetoothUART_IntClock_CLKEN |= BluetoothUART_IntClock_CLKEN_MASK;
	BluetoothUART_IntClock_CLKSTBY |= BluetoothUART_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: BluetoothUART_IntClock_Stop
********************************************************************************
*
* Summary:
...
void BluetoothUART_IntClock_Stop(void) 
{
    ...
    BluetoothUART_IntClock_CLKEN &= (uint8)(~BluetoothUART_IntClock_CLKEN_MASK);
	BluetoothUART_IntClock_CLKSTBY &= (uint8)(~BluetoothUART_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: BluetoothUART_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((BluetoothUART_IntClock_CLKEN & BluetoothUART_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = BluetoothUART_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = BluetoothUART_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* BluetoothUART_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(BluetoothUART_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(BluetoothUART_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: BluetoothUART_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: BluetoothUART_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = BluetoothUART_IntClock_GetSourceRegister();
    uint16 oldDivider = BluetoothUART_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = BluetoothUART_IntClock_CLKEN & BluetoothUART_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(BluetoothUART_IntClock_DIV_PTR, clkDivider);
                BluetoothUART_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                BluetoothUART_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(BluetoothUART_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((BluetoothUART_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(BluetoothUART_IntClock_DIV_PTR, clkDivider);
				BluetoothUART_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: BluetoothUART_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(BluetoothUART_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: BluetoothUART_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    BluetoothUART_IntClock_MOD_SRC |= modeBitMask & (uint8)BluetoothUART_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: BluetoothUART_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    BluetoothUART_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(BluetoothUART_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: BluetoothUART_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return BluetoothUART_IntClock_MOD_SRC & (uint8)(BluetoothUART_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: BluetoothUART_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = BluetoothUART_IntClock_GetDividerRegister();
    uint8 oldSrc = BluetoothUART_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        BluetoothUART_IntClock_MOD_SRC |= CYCLK_SSS;
        BluetoothUART_IntClock_MOD_SRC =
            (BluetoothUART_IntClock_MOD_SRC & (uint8)(~BluetoothUART_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: BluetoothUART_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return BluetoothUART_IntClock_MOD_SRC & BluetoothUART_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: BluetoothUART_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    BluetoothUART_IntClock_PHASE = clkPhase & BluetoothUART_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: BluetoothUART_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return BluetoothUART_IntClock_PHASE & BluetoothUART_IntClock_PHASE_MASK;
}
